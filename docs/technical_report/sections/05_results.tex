...
BF16 and TE use local batch size 2 with eight gradient-accumulation steps
(LB2/GA8); the custom paths use local batch size 4 with four
gradient-accumulation steps (LB4/GA4).
...
\begin{table}[H]
  ...
  uses the selective-8 setting, which checkpoints every eighth transformer
  block; all other rows use selective-32, which checkpoints every 32nd block.
  ...
  \begin{tabular}{ll@{\qquad}rrr}
    ...
    Custom global \nvfp{} & 4/4 & 32,490.50 & 75.27\% & 155.63 GiB \\
    \mxfp{} + row-SR & 4/4 & 37,876.75 & 87.74\% & 154.43 GiB \\
    \localcta{} & 4/4 & 33,243.76 & 77.01\% & 115.52 GiB \\
...
Under the same 32-GPU geometry with local batch size 4 and four gradient
accumulation steps (LB4/GA4), the
...
has higher loss. Reported comparisons use exact shared logged steps; no
reported loss value is interpolated. The dashed custom-global-NVFP4 line is the
display-only fit described below.
...
\begin{figure}[tbp]
  ...
  plotted endpoint. Custom-global-\nvfp{} has regular observations through
  step 19,000, but its dense later history was not retained. The upper panel
  therefore shows the 74 exact later observations recovered from training
  logs, including the terminal diamond; the lower panel shows the 18
  observations recorded at the same step as BF16. The dashed
  custom-global-\nvfp{} line is a smoothed visual guide through those sparse
  observations, not additional measurements. Appendix~\ref{app:reproducibility}
  records the surviving data sources. The 27/5 depth hybrid
  joins three logged training segments at exact checkpoint boundaries, with
...
\begin{figure}[tbp]
  ...
  \caption{Training loss from 120B to 160B tokens. Diamonds mark exact logged
  endpoints. The 27/5 curve uses its complete logged history without
  interpolation. As in Figure~\ref{fig:llama8b-loss}, the late
  custom-global-\nvfp{} markers are exact observations and the dashed curve is
  a smoothed visual guide, not additional measurements.}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{Yrr}
    ...
    27/5 depth hybrid & 2.476842 & 4.75\% \\
...
To compare error in the backward input-gradient calculation (Dgrad), we used
tensors captured at step 28,000 from the \mxfp{}+row-SR+fixed-H32 run. We
recomputed Dgrad with \localcta{} and with \mxfp{}, using BF16 as the
reference. \localcta{} had lower Dgrad error energy in all 288
layer--component comparisons; the geometric-mean
...
on one fixed external stream. We report mean negative log-likelihood (NLL),
for which lower values are better. At the common step-38,000 cut, \mxfp{} with fixed H32 has
...
\localcta{} and 2.462074 for custom global \nvfp{}. Relative to BF16, the
relative NLL differences $100(L_{\mathrm{BF16}}-L_{\mathrm{route}})/L_{\mathrm{BF16}}$
for \mxfp{}-H32, four-final-BF16-block, and operand-H32 are
$+0.305\%$, $-0.299\%$, and $-1.384\%$, respectively; positive values
indicate lower NLL than BF16. These comparisons cover complete recipes from
one training trajectory each; they do not isolate the effect of H32 or average
over repeated seeds.

The 27/5 depth hybrid is evaluated at the five common cuts: steps 2,000,
10,000, 18,000, 29,000, and 38,000. At step 38,000 its validation NLL is
2.538116, or 4.440\% higher than matched BF16.
...
\begin{figure}[tbp]
  ...
  evaluated checkpoints only; no value is interpolated or extrapolated.}
