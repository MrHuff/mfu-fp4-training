...
reported loss value is interpolated. The v5 line is the display-only fit
described below.
...
\begin{figure}[tbp]
  ...
  (LOWESS) fit. The 27/5 depth hybrid joins three logged training segments at
...
\begin{figure}[tbp]
  ...
  interpolation. As in Figure~\ref{fig:llama8b-loss}, the sparse late-v5
  observations are shown by a display-only LOWESS fit.}
