...
\begin{figure}[tbp]
  ...
  plotted endpoint. Custom-global-NVFP4 telemetry is dense through step 19,000. After that
  point, the upper panel marks 74 exact recovered observations, including
  the terminal diamond; the lower panel marks the 18 observations with an
  exact BF16 step match. Dashed lines show a display-only robust locally
  weighted scatterplot smoothing (LOWESS) fit evaluated on the common BF16
  display grid. The 27/5 depth hybrid joins three logged training segments at
  exact checkpoint boundaries, with no interpolated values.}
...
\begin{figure}[tbp]
  ...
  custom-global-NVFP4 markers are exact observations and the dashed curve is
  their display-only dense-grid
  LOWESS fit.}
...
indicate lower NLL than BF16. These are observed
