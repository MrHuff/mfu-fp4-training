...
Fusion itself is established systems practice. The contribution here is the
FP4 boundary contract: a producer completes the format's scale domain, emits
both physical orientations needed by Fprop, Dgrad, and Wgrad, and retains the
BF16 and random state required by autograd. Those obligations differ across
MXFP4, global NVFP4, and CTA-local NVFP4, so one generic quantize--GEMM fusion
schedule cannot represent all three paths.
