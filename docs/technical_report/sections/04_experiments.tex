...
weights, scales, and the separate Wgrad-oriented gradient representation
remain deterministic in every custom route labelled ``row SR.'' A Wgrad
Hadamard transform always acts on $X$ and $dY$ as a pair; it never transforms
the stored weight.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}YYYYYY@{}}
    ...
    Custom global \nvfp{} & blocks 1--32 & global NV & global NV & global NV & custom 2D16 \\
    \mxfp{} + row-SR & blocks 1--32 & MX & MX & MX & custom 2D32 \\
    \localcta{} & blocks 1--32 & localCTA & localCTA & localCTA & custom 2D16 \\
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}lYYY@{}}
    \toprule
    Route & Gradient rounding & Wgrad preconditioning & BF16 body blocks / output head \\
    \midrule
    ...
    Custom global \nvfp{} & row data SR & none & none / BF16 \\
    \mxfp{} + row-SR & row data SR & none & none / BF16 \\
    \localcta{} & row data SR & none & none / BF16 \\
...
``TE-managed'' means that Transformer Engine controls the H16 transform. Its
sign-state contract is distinct from the explicit fixed-sign contract used by
the custom kernels.
...
Every operand marked ``MX'' in Table~\ref{tab:recipe-ledger} uses
encode-centric E8M0 scales for activation and gradient blocks and the adaptive
2D32 weight rule in Section~\ref{sec:scale-contracts}. The scale exponent
itself is deterministic. ``Row SR'' applies to the E2M1 gradient values, not
to their E8M0 scale. This distinction is part of the recipe: scale direction
controls range and grid spacing, whereas data SR controls how a value is
selected between adjacent E2M1 levels.

Appendix~\ref{app:reproducibility} gives the checkpoint inclusion criteria and
...
and per-GPU timing records. Each route uses its selected local batch,
...
and shuffled before packing so one GPU worker cannot receive examples from
only one source or contiguous region of source data. Every checkpoint consumes
the same tokenized sequences. We
...
Before route comparison, the converted-checkpoint evaluator and native
TorchTitan model must agree at the full 8,192-token context, and a late BF16
checkpoint must improve over an early one by more than five sequence-level
standard errors.
...
before scoring. The evaluation panel follows the four-task subset used by the
reference NVFP4 protocol: MMLU, HellaSwag, WinoGrande, and ARC-Challenge
