...
concrete delta evaluated here.

\begin{table}[tbp]
  \centering
  \footnotesize
  \caption{Established numerical mechanisms and the concrete system delta
...
levels differ by exactly two. The reported \mxfp{}-v4 routes use an
encode-centric scale for activation and gradient blocks. For block maximum
$a_b=\max_i|z_i|$, they compute
\begin{equation}
...
\end{equation}
Rounding the exponent upward keeps the block maximum inside the finite E2M1
range. A decode-centric alternative rounds the exponent downward, yielding a
denser grid at the cost of possible clipping; it is implemented but is not
used by the reported \mxfp{} activation or gradient paths. The orientation-consistent
2D32 weight producer uses a separate adaptive rule: it starts from the safe
upward-rounded scale $s_T$ for the maximum $a_T$ over a $32\!\times\!32$
weight tile, then selects $s_T/2$ for a nonzero normal tile when
$a_T/s_T<0.87$. The
calibrated threshold trades limited tile-endpoint clipping for a grid twice as
dense. E8M0 scale stochastic rounding is disabled in these recipes.
``Row-SR'' elsewhere in the paper instead denotes stochastic rounding of the
E2M1 gradient payload.

\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.96\textwidth]{figures/fp4_mxfp4_scale_rounding.png}
  \caption{Encode- and decode-centric E8M0 scale selection. Rounding the block
  maximum upward preserves the E2M1 endpoint. For a non-power-of-two maximum,
  rounding it downward halves the decoded grid spacing but can clip large
  values. The reported activation and gradient paths use the encode-centric
  rule. Their 2D32 weight path selects the smaller scale only under the
  calibrated $a_T/s_T<0.87$ condition.}
