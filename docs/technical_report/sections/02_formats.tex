...
concrete delta evaluated here. LUQ means \emph{logarithmic unbiased
quantization}: it makes logarithmic neural-gradient quantization unbiased by
treating underflow probabilistically, rounding between adjacent logarithmic
levels, and deriving the range from the tensor maximum so no value clips at
the upper endpoint \cite{chmiel2023accurate}. Here, \emph{saturation} means
clipping a magnitude above the scaled E2M1 maximum, $q_{\max}$, to
$q_{\max}$. Because there is no representable upper neighbor, this clipping
...
\begin{table}[tbp]
  ...
  \begin{tabularx}{\textwidth}{@{}lYY@{}}
    ...
    \midrule
    SR rule
      & Logarithmic unbiased quantization (LUQ) maps a sub-minimum magnitude
        probabilistically to zero or $q_{\min}$, rounds in-range values between
        adjacent logarithmic levels, and derives the range from $\max|x|$ to
        avoid saturation (clipping above $q_{\max}$)
        \cite{chmiel2023accurate}
      & Our range follows each route's microscale contract. Finite E2M1
        neighbors use the same distance-proportional SR rule; out-of-range
        values saturate by clipping to $q_{\max}$. Rank-specific RNG state is
        fused into the producer and preserved across resumes \\
    SR placement
