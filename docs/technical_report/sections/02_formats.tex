...
\begin{table}[tbp]
  ...
  \begin{tabularx}{\textwidth}{@{}lYY@{}}
    ...
    \midrule
    SR rule
      & FP4 All the Way uses distance-proportional SR between adjacent
        representable values for backward and update quantization
        \cite{chmiel2025fp4alltheway}. At the upper endpoint, a larger scaled
        magnitude saturates by clipping to $q_{\max}$
      & We use the same adjacent-value probability on finite E2M1 bins, while
        the representable range follows each route's microscale contract.
        Rank-specific RNG state is fused into the producer and preserved across
        resumes \\
    SR placement
