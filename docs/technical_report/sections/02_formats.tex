...
cim2026nativemxfp4}. Our methodological contribution specializes where these
...
\begin{table}[tbp]
  \centering
  \footnotesize
  \caption{Established numerical and systems mechanisms and the exact
  project-specific parameterization, composition, or implementation delta
  used in this work.}
  \label{tab:prior-work-boundary}
  \begin{tabularx}{\textwidth}{@{}lYY@{}}
    \toprule
    Component & Established method & Exact project delta \\
    \midrule
    MXFP4 scale policy
    ...
    2D weight contract
      & Orientation-consistent 2D16 NVFP4 and 2D32 MXFP4 encodings are prior
        art
      ...
      & Pure-format and depth-wise routes preserve one weight encoding across
        Fprop and Dgrad. Global v5 returns both packed views under one 2D16
    ...
    SR placement
      & Adjacent-bin SR is established. Chmiel et al. use it in Dgrad and on
    ...
    Wgrad preconditioning
      & Paired randomized and deterministic Hadamard transforms are prior art
    ...
    Format-aware fusion
      ...
      & Fusion follows scale completion. QKV/RoPE and gated-FFN producers
        co-produce consumer layouts and retained backward state; global v5
...
\begin{figure}[tbp]
  ...
  route makes this established range--resolution choice explicit per operand:
...
\begin{figure}[tbp]
  ...
  different numerical weights. Orientation-consistent 2D quantization---an
  established numerical principle---computes one tile encoding and exposes it
  through row- and column-oriented physical descriptors. Our contribution is
  the fused production of those descriptors in the custom MXFP4 and CTA-local
  routes under their respective scale policies.}
...
\cite{rahimifar2026transposition,cim2026nativemxfp4}.

The 2D invariant itself is therefore not our claim. Our contribution is its
format-dependent fused realization in the complete training path. The MXFP4
and CTA-local producers each load the BF16 weight tile, complete the route's
scale reduction, quantize one numerical encoding, and emit the row- and
column-oriented physical layouts together. The MXFP4 producer incorporates
...
Neither the SR estimator nor Hadamard preconditioning is new. Our contribution
is the consumer-specific composition and its fused realization: the row-facing
$dY$ representation uses SR for Dgrad, whereas a separate deterministic
column-facing representation is paired with transformed $X$ for Wgrad. The
producer performs the fixed-sign transform together with column quantization;
Dgrad and the stored weight are not transformed. H16 matches the 16-value
NVFP4 scale block, while H32 matches the 32-value MXFP4 scale block. This
placement integrates established deterministic Hadamard preconditioning
\cite{cim2026nativemxfp4} with axis-selective SR and the format-specific
producer paths without claiming a new stochastic estimator or transform. The
