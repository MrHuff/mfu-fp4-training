...
We abbreviate the forward, input-gradient, and weight-gradient matrix
multiplications as Fprop, Dgrad, and Wgrad, respectively.
...
Stochastic rounding (SR) can remove within-bin rounding bias, while its placement
and treatment of underflow and saturation---clipping to the largest
representable magnitude---remain part of the quantizer
...
E2M1 denotes the four-bit value format with one sign bit, two exponent bits,
and one fraction bit. E8M0 is a power-of-two scale format, E4M3 is an
eight-bit floating-point scale format, and \emph{amax} denotes the maximum
absolute value. A 2D16 or 2D32 weight encoding shares one numerical scale over
a $16\times16$ or $32\times32$ tile and reuses it when the weight is viewed in
either orientation. H16 and H32 denote Hadamard transforms that mix 16 or 32
values. Table~\ref{tab:prior-work-boundary} pairs each existing method with its
implementation in this work; the following subsections explain these choices
in detail.

\begin{table}[tbp]
  \centering
  \footnotesize
  \caption{Existing numerical and systems methods and their parameterization,
  composition, or implementation in this work.}
  \label{tab:prior-work-boundary}
  \begin{tabularx}{\textwidth}{@{}lYY@{}}
    \toprule
    Component & Existing method & Implementation in this work \\
    \midrule
    MXFP4 scale policy
      & E8M0 forces the block scale to a power of two. Rounding the exponent
        up keeps the block maximum within the largest finite E2M1 magnitude,
        6; rounding
    ...
    CTA-local outer scaling
      ...
      & We factor the outer correction into FP32 row- and column-tile factors
        ...
        Tensor Core accumulation, replacing the tensor-wide maximum-magnitude
        reduction
        with tile-local scale completion \\
    2D weight contract
      & NVFP4 uses orientation-consistent 2D16 weights, and existing MXFP4
      ...
      & Pure-format and depth-wise routes preserve one weight encoding across
        ...
        the same BF16 tile \\
    Stochastic-rounding placement
      & Chmiel et al. use adjacent-bin stochastic rounding in Dgrad and on
        both Wgrad operands; NVIDIA applies gradient SR to both Dgrad and Wgrad
        \cite{chmiel2025fp4alltheway,nvidia2025nvfp4}
      & One producer creates two consumer-specific $dY$ representations: a
        stochastically rounded row-facing operand for Dgrad and a deterministic column-facing
        representation for Wgrad \\
    Wgrad preconditioning
      & Existing FP4 recipes use paired randomized or deterministic Hadamard
        transforms
        \cite{tseng2025mxfp4,nvidia2025nvfp4,cim2026nativemxfp4}
      & In signed-Hadamard routes, fixed-sign H16/H32 reuses the fixed
        16-entry sign pattern from the pinned TE implementation; the plain-H16
        hybrid uses no sign flips. Each acts only on paired $X,dY$ Wgrad
        representations and is fused with column quantization. Dgrad and
    ...
    Format-aware fusion
      ...
      & The fusion boundary is set by when the required scale becomes available.
        Query/key/value, rotary-position, and gated feed-forward stages produce
        packed layouts and saved backward tensors together; custom global NVFP4
        overlaps tensor amax while CTA-local removes that dependency \\
    Operand-wise hybrid
      & Existing recipes vary precision by layer or module
      ...
      & We assign MXFP4 or CTA-local NVFP4 per contraction inside every linear
        and produce both representations together from shared BF16 inputs, motivated
        by comparing Dgrad error on identical saved tensors \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[htbp]
  ...
  changes the outer-scale domain while retaining E2M1 values and E4M3
  block scales.}
...
reported quantitative comparison. Accordingly, every reported \mxfp{}
experiment rounds activation and gradient block scales upward, as shown in
Equation~\ref{eq:mxfp4-encode-scale}. This choice protects the largest value
in each block from clipping. The weight rule described next is the only place
where our \mxfp{} recipe sometimes selects the finer downward scale.
...
This tensor-wide maximum also creates a scheduling dependency. A GPU divides
the tensor among many cooperative thread arrays (CTAs), which are thread
blocks that may run at different times. Each CTA can find a partial maximum,
but the final tensor maximum is unavailable until all partial results have
been combined. Packing the quantized tensor and starting the dependent matrix
multiplication must wait for that result. The wait forms a tensor-wide
synchronization boundary within the GPU and can leave a latency bubble even
when individual tiles are otherwise ready.

The CTA-local route removes that cross-CTA dependency by changing only the
outer-scale domain. Each CTA computes maxima for the row and column tiles it
already owns, derives their outer factors using synchronization within that
CTA, and can then proceed to its matrix-multiplication tile without waiting
for unrelated CTAs. It does not remove synchronization altogether; it confines
scale completion to one CTA. For one matrix-multiplication output tile, an
A-row tile owns factor $\alpha_i$ and a B-column tile owns factor $\beta_j$.
...
\begin{figure}[tbp]
  ...
  CTA-local NVFP4 computes tile-owned row and column outer factors without
  waiting for a tensor-wide reduction. The completion point determines which
  preparation work can be fused with its matrix multiplication.}
...
\end{align}
Thus one saved gradient needs a row-oriented representation for Dgrad and a
column-oriented representation for Wgrad; weights likewise participate in two
...
\begin{figure}[H]
  ...
  \caption{Formats and packed views for the plain-H16 operand hybrid. Here
  $r$ and $c$ denote the consumer-facing row- and column-packed scale/layout
  representations; they do not change a tensor's logical shape. Top: Fprop
  consumes row-facing $X$ and $W$, Dgrad consumes row-facing $dY$ and
  column-facing $W$, and Wgrad consumes column-facing $dY$ and $X$. Bottom:
  Fprop uses MXFP4 $X_r$ and 2D32 $W_r$; Dgrad uses CTA-local NVFP4
  $dY_r$ with row stochastic rounding (SR) and 2D16 $W_c$; Wgrad uses MXFP4
  $dY_c$ and $X_c$ with paired plain-H16 preconditioning. One fused backward
  producer emits both $dY$ representations from the BF16 tile. Wgrad does not
  read $W$ because it produces $dW$. Plain H16 has no sign flips; fixed H32
  changes only the paired Wgrad preconditioner.}
...
\end{equation}
even though the two memory layouts contain differently ordered bytes.
...
\begin{figure}[H]
  ...
  through row- and column-oriented memory layouts. Our implementation
  optimization fuses production of those layouts into the custom MXFP4
...
is replicated into the $1\times16$ scale layouts required by the Tensor
...
packed values, block scales, and both physical weight layouts directly.
CTA-local NVFP4 retains the 2D16 consistency invariant. Its weight producer
finds the local weight maxima and E4M3 block scales, folds the relative
corrections into those scales, and publishes one common FP32 weight outer
factor for both orientations. Activations and gradients continue to use the
tile-owned row and column factors in Equation~\ref{eq:localcta}. Our \mxfp{} path applies the same
...
existing systems \cite{rahimifar2026transposition,cim2026nativemxfp4}.
...
CTA-local producer combines its 2D16 encoding with local maximum and scale
calculation, then normalizes both layouts to a common weight outer factor. This construction
...
the stochastically rounded Dgrad input and paired transformed Wgrad inputs in
the fused backward schedule.
...
sign vector shared across linear layers throughout training; the TE
implementation used in our experiments stores a fixed 16-entry vector and caches the resulting
transform \cite{nvidia2025nvfp4,nvidia2026te}. H16 and H32 name the number of
values mixed by the transform.

Our kernels reuse that TE 16-entry sign vector as an explicit route
constant. H16 applies the sign pattern once; H32 repeats it across two 16-value halves
