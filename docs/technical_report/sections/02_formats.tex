...
contraction-specific composition inside complete native training paths; we do
not claim the underlying rounding rule, 2D scaling rule, or Hadamard transform
as new.

In particular, our SR is ordinary adjacent-bin rounding, not Chmiel et al.'s
LUQ construction; its distinctive feature here is that only the row-facing
$dY$ representation consumed by Dgrad is rounded stochastically. The 2D16 and
2D32 scale definitions are prior work; our kernels produce one numerical
weight encoding and both physical consumer layouts at the format's scale
completion boundary. Hadamard mixing is also prior work; our named H16/H32
routes use a fixed sign pattern on the paired Wgrad operands only and are not
presented as RHT. These boundaries separate the borrowed numerical tools from
the fused placement evaluated in this report.

\begin{table}[tbp]
  \centering
  \footnotesize
  \caption{Prior art and the contribution boundary. The right column states
  the route design and fused realization evaluated here, not a claim to have
  introduced the underlying numerical mechanism.}
  ...
  \begin{tabularx}{\textwidth}{@{}lYY@{}}
    ...
    \midrule
    SR rule
      & Ordinary SR samples adjacent values in proportion to distance.
        Chmiel et al.'s LUQ additionally handles logarithmic underflow and
        clipping \cite{gupta2015limited,chmiel2023accurate,ozkara2025sr}
      & E2M1 quantization uses ordinary adjacent-bin SR on selected gradients;
        it is not LUQ and no whole-tensor unbiasedness after saturation is claimed \\
    SR placement
      ...
      & Custom routes apply SR only to row-facing $dY$ for Dgrad; their Wgrad
        carriers remain deterministic, and ranked RNG state survives resumes \\
    2D weights
      & NVIDIA/TE use one 2D16 weight encoding across Fprop and Dgrad; 2D32
        ...
        cim2026nativemxfp4}
      & Custom 2D16 and 2D32 producers fuse one numerical encoding into both
        consumer layouts; the contribution is the native producer, not a new
        2D scale definition \\
    Hadamard preconditioning
      ...
      & Named routes fuse paired, fixed-sign H16 or H32 into Wgrad only. They are
        not labeled RHT, and no random-sign concentration guarantee is claimed \\
    Scale-aware fusion
