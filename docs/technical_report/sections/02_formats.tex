...
formats, and later work showed how unbiased rounding can support 4-bit matrix
multiplications \cite{sun2020ultralow,chmiel2023accurate}. FP8 and microscaling
then established floating-point formats with shared block exponents
...
\cite{tseng2025mxfp4}. NVIDIA's NVFP4 recipe combines two-dimensional weights,
gradient stochastic rounding, paired Wgrad preconditioning, and selected
higher-precision linears \cite{nvidia2025nvfp4}. Quartet and FP4 All the Way
study fully low-precision paths, while subsequent work develops native
...
Stochastic rounding controls systematic rounding bias
\cite{gupta2015limited,ozkara2025sr}. Paired Hadamard preconditioning spreads
outliers while preserving the unquantized dot product
\cite{ashkboos2024quarot,tseng2025mxfp4,nvidia2025nvfp4,
cim2026nativemxfp4}. Our numerical contribution is their contraction-specific
integration, not the mechanisms in isolation.
...
Table~\ref{tab:prior-work-boundary} separates established numerical mechanisms
from the integrations evaluated here. The contribution is their format- and
contraction-specific composition inside complete native training paths.

\begin{table}[tbp]
  \centering
  \footnotesize
  \caption{Relationship to prior work. The left column names the established
  basis; the right column states the integration evaluated in this report.}
  \label{tab:prior-work-boundary}
  \begin{tabularx}{\textwidth}{@{}lYY@{}}
    \toprule
    Component & Established basis & This report \\
    \midrule
    2D weights
      & NVFP4 and transposition-invariant quantization share one numerical
        encoding across forward and backward orientations
        \cite{nvidia2025nvfp4,rahimifar2026transposition,cim2026nativemxfp4}
      & Native 2D16 and 2D32 producers emit both consumer layouts inside the
        global-NVFP4, CTA-local, and MXFP4 execution paths \\
    Gradient SR
      & Unbiased rounding and backward-gradient SR are established tools for
        low-precision training
        \cite{gupta2015limited,chmiel2025fp4alltheway,ozkara2025sr}
      & Custom routes apply SR only to the row-facing $dY$ consumed by Dgrad
        and retain its ranked random state across fused producers and resumes \\
    Hadamard preconditioning
      & Paired randomized or deterministic transforms suppress outliers while
        preserving the unquantized dot product
        \cite{ashkboos2024quarot,tseng2025mxfp4,nvidia2025nvfp4,
        cim2026nativemxfp4}
      & A paired fixed-sign H32 transform is fused into MXFP4 Wgrad; the named,
        differently configured H16 and H32 routes compare complete recipes
        under MXFP4 and CTA-local scaling \\
    Scale scheduling
      & Standard NVFP4 uses a tensor-global outer scale; MXFP4 uses local
        power-of-two block scales \cite{ocp2023mx,nvidia2025nvfp4}
      & Global v5 overlaps the tensor reduction, while CTA-local NVFP4 factors
        the outer scale by output tile and carries both factors into GEMM \\
    Fusion and overlap
      & IO-aware kernels and GEMM--epilogue programs optimize complete
        producer--consumer boundaries
        \cite{dao2022flashattention,hsu2024liger,spector2024thunderkittens,
        guo2026coda}
      & RMSNorm, QKV, gated-FFN, residual, quantization, dual-layout, and saved
        backward work are fused or overlapped according to scale completion \\
    Hybrid assignment
...
\begin{equation}
  \widehat z_i = S\,s_{b(i)}
  Q_{\mathrm{E2M1}}\!\left(\frac{z_i}{S\,s_{b(i)}}\right),
  \label{eq:hierarchical-quant}
\end{equation}
where $s_{b(i)}$ is a block scale, $S$ is an optional outer scale, and
...
TE and global v5 use the tensor-level NVFP4 outer scale; global v5 emits the
...
\cite{fitzgibbon2025fewbits}. NVIDIA's
recommended NVFP4 recipe uses round-to-nearest for weights and activations and
SR for gradient inputs to both Dgrad and Wgrad \cite{nvidia2025nvfp4}.  Our
custom long routes are intentionally narrower: data SR is enabled only on the
row-oriented $dY$ representation consumed by Dgrad. The Wgrad column
representation, weights, activations, and all scale values remain
deterministic.
...
\cite{nvidia2025nvfp4,nvidia2026te}. H16 and H32 name the number of values
mixed by the transform.

Our custom kernels use one fixed sign diagonal: the H16 path uses the
16-entry pattern in the referenced TE implementation, and H32 repeats that
motif over two 16-value halves. We therefore call these paths \emph{fixed-sign
H16/H32 preconditioning}, rather than RHT. The additions and subtractions mix
