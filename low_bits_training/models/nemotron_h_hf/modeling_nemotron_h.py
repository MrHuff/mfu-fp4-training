...
_native_mamba_ok, _native_mamba_error = native_mamba_available()
_mamba_fast_path_missing = [] if _native_mamba_ok else [_native_mamba_error]

if is_flash_attn_2_available():
    from transformers.modeling_flash_attention_utils import _flash_attention_forward

is_fast_path_available = _native_mamba_ok


def _mamba_gated_out_projection(
    out_proj,
    norm,
    scan_output,
    gate,
    *,
    ...
    fused_forward = getattr(out_proj, "forward_mamba_gated", None)
    fused_enabled = (
        os.environ.get("LBT_NEMOTRON_H_FUSED_MAMBA_GATED_OUT_PROJ", "0")
        != "0"
    )
    if callable(fused_forward) and fused_enabled:
        if residual is not None or cde_emit:
            raise RuntimeError(
                "fused gated Mamba projection does not support the "
                "residual/CDE epilogue contract"
            )
        if int(norm.group_size) != 1024:
            raise RuntimeError(
                "fused Nemotron-H Mamba output projection requires "
                f"group_size=1024, got {norm.group_size}"
            )
        if not getattr(
            ...
            logger.info(
                "Nemotron-H fused Mamba gated RMS -> FP4 output projection "
                "fired: backend=%s scan=%s gate_stride=%s",
                type(out_proj).__name__,
                tuple(scan_output.shape),
                tuple(gate.stride()),
            )
            _mamba_gated_out_projection._fused_route_logged = True
    ...
    if (
        ...
        raise RuntimeError(
            "the production recipe requires fused Nemotron-H Mamba gated "
            f"output projection, but {type(out_proj).__name__} does not "
            "provide forward_mamba_gated"
        )
    normed = norm(scan_output, gate)
    if residual is not None or cde_emit:
        return out_proj(normed, residual=residual, cde_emit=cde_emit)
...
def _linear_bias_or_none(module: nn.Module) -> Optional[torch.Tensor]:
...
def _can_fuse_dense_out_proj(module: nn.Module) -> bool:
    # The Mamba fused training kernel consumes outproj_weight directly, which
    # bypasses custom/TE Linear.forward quantization. Fuse only the exact dense
    # nn.Linear case; quantized Linear subclasses must run their own forward.
...
class MambaRMSNormGated(torch.nn.Module):
    ...
    def forward(self, hidden_states, gate=None):
        if gate is None:
            raise RuntimeError("Nemotron-H native gated RMSNorm requires a gate tensor")
        return gated_rmsnorm_cuda(
            hidden_states, gate, self.weight, self.variance_epsilon, self.group_size
        )
...
class NemotronHMamba2Mixer(nn.Module):
    ...
    def __init__(self, config: NemotronHConfig, layer_idx: int):
        ...
        if not is_fast_path_available:
            logger.warning_once(f"Pure CUDA Nemotron-H Mamba path unavailable: {_native_mamba_error}")
    ...
    def cuda_kernels_forward(
        ...
        if not self.training or cache_params is not None:
            raise RuntimeError(
                "pure CUDA Nemotron-H Mamba currently supports full-sequence training only"
            )
        if self.time_step_limit != (0.0, float("inf")):
            raise RuntimeError(
                "pure CUDA selective scan does not implement a finite dt_limit; "
                f"got {self.time_step_limit}"
            )
        ...
        if cache_params is not None and cache_position is not None and cache_position[0] > 0:
            raise RuntimeError("pure CUDA Nemotron-H path currently supports training sequences, not token-cache updates")
        ...
        # Fused calculations or step by step if no initialized cache is found
            ...
            if self.training and cache_params is None:
                ...
                hidden_states_B_C = causal_conv1d_cuda(
                ...
                hidden_states, B, C = split_mamba_conv_output(
                    hidden_states_B_C,
                    self.intermediate_size,
                    groups_time_state_size,
                )
                scan_output = selective_scan_mamba2(
                ...
                out = _mamba_gated_out_projection(
                    self.out_proj,
                    self.norm,
                    scan_output,
                    gate,
    ...
    def forward(
        ...
        if residual is not None or cde_row_rms_partial is not None or cde_emit:
            raise RuntimeError(
                "Nemotron Mamba residual/CDE epilogue requires the native CUDA path"
            )
        if "cuda" in self.in_proj.weight.device.type and getattr(self.config, "use_mamba_kernels", True):
            missing = "; ".join(_mamba_fast_path_missing) or "unknown import failure"
            raise RuntimeError(
                "Nemotron-H CUDA Mamba fast path is unavailable. Refusing to use "
                "the torch reference path because it materializes an "
                "O(batch * chunks * chunk_size^2 * heads * state_size) tensor "
                "and OOMs at the 8B/8192-token training shape. Missing kernels: "
                f"{missing}. Install working mamba-ssm and causal-conv1d kernels, "
                "or set use_mamba_kernels=false only for tiny/debug runs."
            )
        dtype = hidden_states.dtype
        if attention_mask is not None and attention_mask.shape[1] > 1 and attention_mask.shape[0] > 1:
