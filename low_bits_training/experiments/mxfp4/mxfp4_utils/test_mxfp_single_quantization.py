...
from quantization.quantization_autograd_functions import E2M1Quantizer
from quantization.MXFPconfig import MXLinearDimConfig


# --- Mock Objects for Demonstration ---
# In your actual code, you would use your real custom modules.
# These mockups are here so the script can be understood and run conceptually.
class ErrorMetrics:
    ...
    def get_stats(self):
        ...
        abs_stats = [
            self.reconstructed_error.mean().item(),
            self.reconstructed_error.median().item(),
            self.reconstructed_error.max().item(),
            self.reconstructed_error.std().item(),
            0.0,
            0.0,
            0.0,  # p99, p999, iqr
        ]
        ...
        if self.original.abs().mean() < 0.001:
            rel_stats = [
                self.relative_error.mean().item() * 500,
                self.relative_error.median().item(),
                self.relative_error.max().item(),
                self.relative_error.std().item(),
                0.0,
                0.0,
                0.0,  # p99, p999, iqr
            ]
...
def plot_reconstruction_metrics(
    df, x_axis, y_axis, hue, title, filename, y_log_scale=False, y_limit=None
):
    """
    ...
    Args:
        ...
        hue (str): The column name for the line colors/styles.
        title (str): The plot title.
        filename (str): The output filename for the saved plot.
        y_log_scale (bool): If True, uses a logarithmic scale for the y-axis.
        y_limit (float, optional): If set, clips the y-axis to this upper bound.
                                 Ignored if y_log_scale is True.
    """
    ...
    if y_limit is not None and not y_log_scale:
        ...
        if num_filtered > 0:
            print(
                f"Plot '{filename}': Note - {num_filtered} of {num_original} points with '{y_axis}' > {y_limit} are outside the visible range."
            )

    sns.lineplot(data=plot_df, x=x_axis, y=y_axis, hue=hue, marker="o", errorbar="sd")
    ...
    is_x_log = (x_axis == "block_size") or (
        plot_df[x_axis].nunique() > 1
        and plot_df[x_axis].max() / plot_df[x_axis].min() > 50
    )
    if is_x_log:
...
def run_reconstruction_experiments(
    ...
    for i, params in enumerate(param_combinations):
        ...
        for alpha, k_val in itertools.product(alpha_list, k_list):
            original_tensor = (
                torch.randn(TENSOR_DIMS, dtype=DTYPE_BASE).to(DEVICE) * tensor_scale
            )
            ...
            config = MXLinearDimConfig(
                ...
                elem_dtype=DTYPE_FP4,  # mock
                scale_type=scale_type,
                fp_scale_factor=fp_scale,
                roundMode=round_mode,
                use_approx={
                    "smooth": ma_method,
                    "alpha": alpha,
                    "stepGradient": step_grad,
            ...
            mx_tensor, _, _ = new_to_mx(
                tensor=tensor, scalingModule=scalingModule, fp4_quantiser=quantiser_FP4
            )
...
if __name__ == "__main__":
    ...
    def create_plot_hue(df):
        df_plot = df.copy()
        df_plot["scale_rounding"] = df_plot["scale_rounding"].apply(
            lambda x: x.split(".")[-1]
        )
        df_plot["plot_hue"] = (
            df_plot["scale_type"]
            + " | "
            + df_plot["scale_rounding"]
            + " | "
            + "tensor scaling:"
            + df_plot["use_tensor_scaling"].apply(
                lambda x: "T" if x else ("F" if not x else np.nan)
            )
        )
        return df_plot
    ...
    exp1_results = run_reconstruction_experiments(
        ...
        scale_rounding_modes=[
            ...
            gfloat.RoundMode.TowardPositive,
        ],
    ...
    if not exp1_results.empty:
        ...
        plot_reconstruction_metrics(
            ...
            "mean_rel_error",
            "plot_hue",
            "STE: Relative Error vs. Block Size",
            "plot_exp1_ste_vs_block_size",
        )
        plot_reconstruction_metrics(
            df_plot_1,
            "block_size",
            ...
            "plot_hue",
            "STE: Relative Error vs. Block Size",
            "plot_exp1_ste_vs_block_size_median",
        )
    ...
    if not exp2_results.empty:
        ...
        # The original plot that is hard to read
        # plot_reconstruction_metrics(df=df_plot_2, x_axis="tensor_scale", y_axis="mean_rel_error", hue="plot_hue",title= "Original Plot: STE Relative Error vs. Tensor Scale (Block Size=16)", filename = "plot_exp2_ste_vs_tensor_scale_original")

        # --- SOLUTION 1: Use a logarithmic y-axis ---
        # This is the best way to see all data when it spans several orders of magnitude.
        plot_reconstruction_metrics(
            df=df_plot_2,
            x_axis="tensor_scale",
            y_axis="mean_rel_error",
            hue="plot_hue",
            title="Log Y-Axis: STE Relative Error vs. Tensor Scale (Block Size=16)",
            filename="plot_exp2_ste_vs_tensor_scale_log_y",
            y_log_scale=True,  # <<< Enable log scale for the y-axis
        )
        plot_reconstruction_metrics(
            df=df_plot_2,
            x_axis="tensor_scale",
            y_axis="median_rel_error",
            hue="plot_hue",
            title="Log Y-Axis: STE Relative Error vs. Tensor Scale (Block Size=16)",
            filename="plot_exp2_ste_vs_tensor_scale_log_y_median",
            y_log_scale=True,  # <<< Enable log scale for the y-axis
        )

        # # --- SOLUTION 2: Clip the y-axis to focus on the non-exploding lines ---
        # # This is useful for "zooming in" on the lower-error lines. The appropriate limit
        # # depends on your data; we'll choose 0.2 as an example.
        # plot_reconstruction_metrics(
        #     df=df_plot_2, x_axis="tensor_scale", y_axis="mean_rel_error", hue="plot_hue",
        #     title="Clipped Y-Axis: STE Relative Error vs. Tensor Scale (Block Size=16)",
        #     filename="plot_exp2_ste_vs_tensor_scale_clipped_y",
        #     y_limit=2.  # <<< Set an upper bound for the y-axis
        # )

    # The remaining experiments are left as they were
    # ... (Experiments 3, 4, 5 would go here) ...
    ...
    exp3_results = run_reconstruction_experiments(
        ...
        step_gradients=["STE"],
        a_values=[40.0],
        k_values=[5.0],  # Use a representative alpha
        scale_quant_grads=["STE"],
        scale_rounding_modes=[
            gfloat.RoundMode.TiesToEven,
            gfloat.RoundMode.TowardPositive,
    ...
    if not exp3_results.empty:
        ...
        plot_reconstruction_metrics(
            ...
            "mean_rel_error",
            "plot_hue",
            r"Softmax: Relative Error vs. Block Size ($\beta$=10)",
            "plot_exp3_softmax_vs_block_size",
        )
        plot_reconstruction_metrics(
            df_plot_3,
            "block_size",
            ...
            "plot_hue",
            r"Softmax: Relative Error vs. Block Size ($\beta$=10)",
            "plot_exp3_softmax_vs_block_size_median",
        )
    ...
    if not exp4_results.empty:
        ...
        # The original plot that is hard to read
        # plot_reconstruction_metrics(df_plot_4, "tensor_scale", "mean_rel_error", "plot_hue", "Original Plot: STE Relative Error vs. Tensor Scale (Block Size=16)", "plot_exp4_ste_vs_tensor_scale_original")
        ...
        plot_reconstruction_metrics(
            ...
            filename="plot_exp4_softmax_vs_tensor_scale_log_y",
            y_log_scale=True,  # <<< Enable log scale for the y-axis
        )
        plot_reconstruction_metrics(
            df=df_plot_4,
            ...
            hue="plot_hue",
            title=r"Log Y-Axis: Softmax Relative Error vs. Tensor Scale (Block Size=16, $\beta=40$)",
            filename="plot_exp4_softmax_vs_tensor_scale_log_y_median",
            y_log_scale=True,  # <<< Enable log scale for the y-axis
        )
        ...
        #     title=r"Clipped Y-Axis: Softmax Relative Error vs. Tensor Scale (Block Size=16, $\beta=40$)",
        #     filename="plot_exp4_softmax_vs_tensor_scale_clipped_y",
        #     y_limit=2.  # <<< Set an upper bound for the y-axis
        # )
    ...
    if not exp5_results.empty:
        ...
        # Note: 'alpha' is aliased to 'a_values' in the function call
        plot_reconstruction_metrics(
            df_plot_5,
            r"$\beta$",
            "mean_rel_error",
            "plot_hue",
            r"Softmax: Relative Error vs. $\beta$ (Block=16, Scale=1.0)",
            "plot_exp5_softmax_vs_beta",
        )
        plot_reconstruction_metrics(
            df_plot_5,
            r"$\beta$",
            ...
            "plot_hue",
            r"Softmax: Relative Error vs. $\beta$ (Block=16, Scale=1.0)",
            "plot_exp5_softmax_vs_beta_median",
        )
