        ...
        **kwargs,
        ...
        if train_loss is not None:
            time_since_last = current_time - (
                self.last_log_time if self.last_log_time else current_time
            )
            avg_step_time = (
                (time_since_last / self.logging_steps) if self.last_log_time else 0.0
            )
...
class CustomOptimizerTrainer(Trainer):
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        if self.optimizer_eval_string:
            try:
                ...
                raise ValueError(
                    f"Error evaluating optimizer_eval_string: '{self.optimizer_eval_string}'. Error: {e}"
                )
        else:
    ...
    def evaluation_loop(
        ...
        all_batch_sizes = EvalLoopContainer(
            self.args.eval_do_concat_batches, padding_index=-100
        )
        metrics = None
        eval_set_kwargs = {}
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Update containers
            tensor_observed_batch_size = torch.tensor(
                observed_batch_size, device=self.args.device
            )
            global_observed_batch_size = self.gather_function(
                (tensor_observed_batch_size)
            )
...
def compute_metrics(eval_pred):
    ...
    return {"eval_loss": (weights * logits).sum().item() / weights.sum().item()}
...
class LlamaTrainerWrapperBigger:
    ...
    def __init__(
        ...
        self.max_tokens = (
            param_count * 100
            if model_type in ["llama_9M", "llama_60M"]
            else param_count * 42
        )
    ...
    def load_and_tokenize_data(self):
        ...
        if os.path.exists(cached_train_path) and os.path.exists(cached_test_path):
            if is_main_process:
                print("✅ Found pre-processed data cache.")
        else:
            ...
            if is_main_process:
                print(
                    "⚠️ No cached data found. Processing data from scratch on main process..."
                )
                try:
                    raw_ds = load_dataset(
                        self.dataset_name,
                ...
                token_stream = truncate_token_lists(
                    self.max_tokens, flat_token_ds["tokens"]
                )
                ...
                input_ids = (
                    token_stream_np[: total_chunks * seq_len].reshape(
                        (total_chunks, seq_len)
                    )
                    if total_chunks > 0
                    else np.full((1, seq_len), eos_id, dtype=np.int32)
                )
        ...
        epochs_needed = float(self.max_tokens * 0.9) / float(
            len(self.train_dataset) * self.sequence_length
        )
        self.num_epochs = max(1, epochs_needed)
        if is_main_process:
            print(f"Total epochs for training: {self.num_epochs:.2f}")
            print(
                f"Validation dataset shrunk from {original_val_size} to {len(self.val_dataset)} samples."
            )
    ...
    def setup_trainer(self):
        ...
        steps_per_epoch = math.ceil(
            num_training_samples / (batch_size * world_size * gradient_accumulation_steps)
        )
    ...
    def run(self, config=None):
        if "LOCAL_RANK" in os.environ and not dist.is_initialized():
            ...
            timeout = (
                ...
                else datetime.timedelta(minutes=1200)
            )
        ...
        try:
            ...
            if is_main_process:
                print(
                    f"⚠️ torch.compile or patch failed: {e}. Running without compilation."
                )
