...
class FFHQDataset(Dataset):
    def __init__(self, root_dir="./data", resolution=128, mode="train"):
        ...
        print("Loading FFHQ 128x128 dataset from Hugging Face Hub or local cache...")
...
class ResBlock(nn.Module):
    def __init__(self, in_channels, out_channels, time_emb_dim):
        ...
        assert (
            in_channels % 8 == 0
        ), f"ResBlock: in_channels ({in_channels}) must be divisible by 8 for GroupNorm."
        ...
        assert (
            out_channels % 8 == 0
        ), f"ResBlock: out_channels ({out_channels}) must be divisible by 8 for GroupNorm."
...
class AttentionBlock(nn.Module):
    ...
    def __init__(self, channels):
        ...
        assert (
            channels % 8 == 0
        ), f"AttentionBlock: channels ({channels}) must be divisible by 8 for GroupNorm."
        ...
        self.proj = nn.Linear(in_features=channels, out_features=channels)
...
class AdaptedUNet(nn.Module):
    def __init__(
        ...
        for i in range(num_down_blocks):
            ...
            self.up_blocks.append(
                nn.ModuleList(
                    [
                        ResBlock(
                            in_channels_resblock, out_channels_transposed, time_emb_dim
                        ),
                        AttentionBlock(out_channels_transposed)
                        if is_attn_up
                        else nn.Identity(),
                        ResBlock(
                            out_channels_transposed, out_channels_transposed, time_emb_dim
                        ),
                        nn.ConvTranspose2d(
                            channels_for_upsample_input,
                            out_channels_transposed,
                            4,
                            stride=2,
                            padding=1,
                        ),
        ...
        self.final_conv = nn.Conv2d(
            channels_for_upsample_input + base_channels, out_channels, 3, padding=1
        )
...
class BigDiffusionTrainer:
    ...
    def save_samples(self, epoch, n_samples=64):
        samples = self.diffusion.sample(
            (n_samples, 3, self.image_resolution, self.image_resolution), self.device
        )
...
class BigGaussianDiffusion(nn.Module):
    ...
    def register_schedule(self, beta_start, beta_end):
        ...
        self.register_buffer(
            "sqrt_one_minus_alphas_cumprod", sqrt_one_minus_alphas_cumprod.to(self.dtype)
        )
    ...
    def q_sample(self, x0, t, noise=None):
        ...
        sqrt_one_minus = self.sqrt_one_minus_alphas_cumprod[t][:, None, None, None].to(
            x0.device
        )
