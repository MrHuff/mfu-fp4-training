...
class ErrorMetrics:
    def __init__(self, out_ref, out):
        ...
        self.relative_error = ((out - out_ref).abs() / (out_ref.abs() + epsilon)).to(
            torch.float32
        )
        ...
        self.iqr_rel = (
            self.relative_error.quantile(0.75) - self.relative_error.quantile(0.25)
        ).item()
        ...
        self.iqr_abs = (
            self.absolute_error.quantile(0.75) - self.absolute_error.quantile(0.25)
        ).item()
    ...
    def get_stats(self):
        return [
            ...
            self.iqr_abs,
            self.rms_error,
            json.dumps(self.worst_offenders_rel),
            json.dumps(self.worst_offenders_abs),
        ]
...
def plot_metrics(df, folder="."):
    ...
    color_palette = sns.color_palette(
        "tab10", len(exmy_values)
    )  # Use Set2 or any other color palette of your choice
    ...
    for metric in metrics:
        ...
        if not special_case.empty:
            for exmy, color in exmy_color_map.items():
                ...
                special_case_metric_values = special_case[special_case["exmy"] == exmy][
                    metric
                ].values
                for val in special_case_metric_values:
                    plt.axhline(
                        ...
                        linestyle="dashed",
                        color=color,
                        label=f"BLOCK_DIM=1 ({metric}, {exmy})",
                    )
        ...
        plt.grid()
        plt.legend(
            title="exmy", bbox_to_anchor=(1.05, 1), loc="upper left", borderaxespad=0.0
        )
...
STEP_GRADIENTS = [
    "STE"
]  # [None, 'baseline', 'spline', 'sigmoid'] #This apparently sucks ass, figure out why and fix it! Cut the bullshit!
...
if __name__ == "__main__":
    ...
    for block_size, scale_type, ma, step_gradient in itertools.product(
        BLOCK_SIZES, SCALE_TYPES, MAX_APPROX, STEP_GRADIENTS
    ):
        ...
        MX_w, _, _ = new_to_mx(tensor=w, scalingModule=scalingModule)
        MX_input, _, _ = new_to_mx(tensor=input_data, scalingModule=scalingModule)
    ...
    for block_size, scale_type, ma, step_gradient in itertools.product(
        ...
        plot_metrics(
            df_results[df_results["max_type"] == "STE"], f"matmul_res_STE_bs={block_size}"
        )
        plot_metrics(
            df_results[df_results["max_type"] == "softsoftmax"],
            f"matmul_res_soft_bs={block_size}",
        )
