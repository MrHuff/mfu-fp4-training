...
import numpy as np
from test_mxfp_matmul import ErrorMetrics
import random
...
class RegressionTrainer:
    def __init__(
        ...
        self.train_data, self.val_data = self._generate_data(
            num_samples, input_dim, noise_std
        )
...
if __name__ == "__main__":
    ...
    for a in [1.0]:
        ...
        for block_size, scale_type, ma, step_gradient, sq_gradient in itertools.product(
            ...
            df_results_w.loc[len(df_results_w)] = [
                block_size,
                scale_type,
                ma,
                step_gradient,
                sq_gradient,
            ] + err_metrics_w.get_stats()
            df_results_X.loc[len(df_results_X)] = [
                block_size,
                scale_type,
                ma,
                step_gradient,
                sq_gradient,
            ] + err_metrics_X.get_stats()
    print(
        df_results_w[
            [
                "block_size",
