...
from scipy.interpolate import CubicSpline,InterpolatedUnivariateSpline,PPoly
import torch
import gfloat
...
class E2M1QuantizerSR(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        rvs = round_ndarray(
            format_info_ocp_e2m1, x, gfloat.RoundMode.Stochastic, sat=True, srbits=srbits, srnumbits=16
        )
...
class STEAbsmax(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        #grad = grad_output.unsqueeze(dim)
        #grad_final = grad.expand_as(x) 
        return grad_output, None
...
class SmoothQuantizeSigmoidOptimizedFunctionFP4(torch.autograd.Function):
    ...
    ], dtype=torch.float32)  # Indexed 0 to 14, 15 elements!
    ...
    def forward(ctx, x, temperature):
        temperature = torch.tensor(temperature, dtype=torch.float32, device=x.device)  # Ensure temperature is a tensor
        
        idx = torch.bucketize(x, SmoothQuantizeSigmoidOptimizedFunctionFP4.e2m1_values) - 1
        idx = torch.clamp(idx, 0, len(SmoothQuantizeSigmoidOptimizedFunctionFP4.centers) - 1)
        
        x_diff = (x - SmoothQuantizeSigmoidOptimizedFunctionFP4.centers[idx]) * SmoothQuantizeSigmoidOptimizedFunctionFP4.sigmoidDeltas[idx]
        sigmoid_values = torch.sigmoid(x_diff / temperature)
        quantized_values = sigmoid_values * SmoothQuantizeSigmoidOptimizedFunctionFP4.e2m1_values[idx + 1] + (1 - sigmoid_values) * SmoothQuantizeSigmoidOptimizedFunctionFP4.e2m1_values[idx]
        
        ctx.save_for_backward(sigmoid_values, idx, temperature)
        return quantized_values

    @staticmethod
    def backward(ctx, grad_output):
        sigmoid_values, idx, temperature = ctx.saved_tensors
        
        grad_input = grad_output * (sigmoid_values * (1 - sigmoid_values)) / temperature * 12
...
class BaselineSmoothQuantFP4(torch.autograd.Function):
    ...
    def forward(ctx, x,  k):
        ...
        k_ = torch.tensor(k, dtype=torch.float32, device=x.device)  # Ensure temperature is a tensor
