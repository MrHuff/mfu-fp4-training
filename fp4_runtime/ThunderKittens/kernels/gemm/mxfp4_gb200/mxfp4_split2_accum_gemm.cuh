#pragma once
// ================================================================
// MXFP4 Split2 One-Pass Accum GEMM
...
namespace mxfp4_split2_accum_gemm {
...
struct config {
    ...
    static_assert(
        !FUSE_H_BWD || (_Nb == 256 && _EPI_PIPE_DEPTH == 8),
    ...
    for (int i = 0; i < RT::height; ++i) {
        ...
        for (int j = 0; j < RT::width; ++j) {
            ...
            #define H_DOT(ROW, COL, V) \
                dot = fmaf( \
                    (__bfloat162float(V) * \
                     __bfloat162float(g.h_gamma[COL])), \
                    __bfloat162float( \
                        g.h_z[static_cast<long long>(ROW) * g.h_cols + \
                              (COL)]), \
                    dot)
            H_DOT(r0, c0, q[0].x);
            H_DOT(r0, c0 + 1, q[0].y);
...
__device__ inline float h_reduce_dot(float value, float *scratch) {
    ...
    if (warp::laneid() == 0) {
        scratch[warpgroup::warpid()] = value;
    }
    warpgroup::sync(1);
    if (warpgroup::warpid() == 0) {
        float total = warp::laneid() < WARPGROUP_WARPS
            ? scratch[warp::laneid()]
            : 0.0f;
        #pragma unroll
        for (int d = 16; d; d >>= 1) {
    ...
    for (int i = 0; i < RT::height; ++i) {
        ...
        for (int j = 0; j < RT::width; ++j) {
            ...
            #define H_APPLY(ROW, COL, V, OUT) do { \
                ...
                const float zv = __bfloat162float( \
                    g.h_z[static_cast<long long>(ROW) * g.h_cols + (COL)]); \
                const float gv = dv * __bfloat162float(g.h_gamma[COL]); \
                OUT = __float2bfloat16_rn(fmaf(-zv, corr, r * gv)); \
            } while (0)
            const float z00 = __bfloat162float(
                g.h_z[static_cast<long long>(r0) * g.h_cols + c0]);
            const float z01 = __bfloat162float(
                g.h_z[static_cast<long long>(r0) * g.h_cols + c0 + 1]);
            const float z10 = __bfloat162float(
                g.h_z[static_cast<long long>(r1) * g.h_cols + c0]);
            const float z11 = __bfloat162float(
                g.h_z[static_cast<long long>(r1) * g.h_cols + c0 + 1]);
            const float z08 = __bfloat162float(
                g.h_z[static_cast<long long>(r0) * g.h_cols + c1]);
            const float z09 = __bfloat162float(
                g.h_z[static_cast<long long>(r0) * g.h_cols + c1 + 1]);
            const float z18 = __bfloat162float(
                g.h_z[static_cast<long long>(r1) * g.h_cols + c1]);
            const float z19 = __bfloat162float(
                g.h_z[static_cast<long long>(r1) * g.h_cols + c1 + 1]);
            ...
            stage[(p + 4) * 2][lr0] =
                __bfloat162float(q[2].x) * z08 * r;
            stage[(p + 4) * 2 + 1][lr0] =
                __bfloat162float(q[2].y) * z09 * r;
            stage[(p + 4) * 2][lr1] =
                __bfloat162float(q[3].x) * z18 * r;
            stage[(p + 4) * 2 + 1][lr1] =
                __bfloat162float(q[3].y) * z19 * r;
            H_APPLY(r0, c0, q[0].x, q[0].x);
            H_APPLY(r0, c0 + 1, q[0].y, q[0].y);
            H_APPLY(r1, c0, q[1].x, q[1].x);
            H_APPLY(r1, c0 + 1, q[1].y, q[1].y);
            H_APPLY(r0, c1, q[2].x, q[2].x);
            H_APPLY(r0, c1 + 1, q[2].y, q[2].y);
            H_APPLY(r1, c1, q[3].x, q[3].x);
            H_APPLY(r1, c1 + 1, q[3].y, q[3].y);
            #undef H_APPLY
        }
    }
}
...
__device__ inline void kernel(const globals<C> &g) {
    ...
    __shared__ float h_reduce[
        C::FUSE_H_BWD ? WARPGROUP_WARPS + 1 : 1];
                    ...
                    for (int q = 0; q < SLICES_PER_TILE; ++q) {
                        ...
                        if (warpgroup::warpid() == 0) {
                            ...
                            for (int w = 0; w < WARPGROUP_WARPS; ++w) {
                                total +=
                                    h_dgamma[w][warp::laneid()][32];
                            }
                            g.h_dgamma_partial[
                                row_tile * g.h_cols + col +
                                warp::laneid()] = total;
                        }
                        warpgroup::sync(1);
                    }
