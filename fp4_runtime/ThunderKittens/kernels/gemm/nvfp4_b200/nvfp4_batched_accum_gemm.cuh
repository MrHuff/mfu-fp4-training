#pragma once
// ================================================================
// NVFP4 Batched GEMM with Fused Accumulation
// D = sum_i(A_i x B_i^T), all batches accumulate into single output.
//
// Internal loops iterate across the batch dimension.
// ================================================================

#include "nvfp4_gemm.cuh"
#include "nvfp4_batched_gemm.cuh"  // for tma_dev_proxy

namespace nvfp4_batched_accum_gemm {

using namespace nvfp4_batched_gemm;
...
struct globals {
    ...
    __host__ inline dim3 grid() const {
        ...
        return dim3(min(spatial_tiles, num_sms()));
    }
...
__device__ inline void kernel_impl(const globals<C> &g) {
    ...
    // Prefetch TMA descriptors for batch 0 initially
    if (threadIdx.x == 0) {
    ...
    if (warpgroup_id >= C::CONSUMER_WARPGROUPS && warp::elect_leader()) {
        ...
        // ── Consumer: scale + accumulate into shared output ──
        ...
        for (int block_idx = cluster_id; block_idx < num_blocks; block_idx += gridDim.x / C::CLUSTER_SIZE) {
            ...
            for (int i = 0; i < C::EPI_PIPE_DEPTH; i++) {
                ...
                warpgroup::sync(1);
                warpgroup::store(output_tiles.D[i%C::NUM_D_TILES], D_reg_bf[i]);
                warpgroup::sync(1);
