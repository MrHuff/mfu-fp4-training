#pragma once
// ================================================================
// NVFP4 localCTA v3 GEMM kernel
// D = A x B^T with tile-global outer scales applied once after accumulation.
// ================================================================
...
__device__ inline void kernel(const globals<C> &g) {
    ...
    if (warpgroup_id >= C::CONSUMER_WARPGROUPS) {
        ...
        for (int block_idx = cluster_id; block_idx < num_blocks; block_idx += gridDim.x / C::CLUSTER_SIZE) {
            ...
            const float gs = g.A_sg_chunks[row_block_idx] * g.B_sg_chunks[col_block_idx];
        ...
        for (int block_idx = cluster_id; block_idx < num_blocks; block_idx += gridDim.x / C::CLUSTER_SIZE) {
            ...
            for (int i = 0; i < num_red_blocks; ++i) {
                ...
                for (int epi = 0; epi < C::EPI_PIPE_DEPTH; ++epi) {
                    ...
                    if (i == 0) {
                        warp::copy(D_acc[epi], D_reg_fl[epi]);
                    } else {
                        warp::add(D_acc[epi], D_acc[epi], D_reg_fl[epi]);
                    }
                }
                warpgroup::sync(1);
                tensor_after_thread_sync();
            ...
            for (int epi = 0; epi < C::EPI_PIPE_DEPTH; ++epi) {
                if (g.silu_dim > 0) {
                    const int col_offset_elems = (C::EPI_PIPE_DEPTH * col_block_idx + epi) * C::Nb / C::EPI_PIPE_DEPTH;
                    if (col_offset_elems < g.silu_dim) {
                        apply_silu_inplace(D_acc[epi]);
                    }
                }
                warp::copy(D_reg[epi], D_acc[epi]);
            }
            ...
            for (int epi = 0; epi < C::EPI_PIPE_DEPTH; ++epi) {
                ...
                warpgroup::sync(1);
                if (g.use_split_D) {
                    const int col_offset = C::EPI_PIPE_DEPTH * col_block_idx + epi;
                    const int col_offset_elems = col_offset * C::Nb / C::EPI_PIPE_DEPTH;
                    if (col_offset_elems < g.q_dim) {
                        warpgroup::tma::store_async<dim::ROW, C::D_CACHE_POLICY>(
                            g.D, output_tiles.D[epi % C::NUM_D_TILES], {row_block_idx*2 + cta_id, col_offset});
                    } else if (col_offset_elems < g.q_dim + g.k_dim) {
                        const int k_col_offset = col_offset - (g.q_dim / (C::Nb / C::EPI_PIPE_DEPTH));
                        warpgroup::tma::store_async<dim::ROW, C::D_CACHE_POLICY>(
                            g.D_K, output_tiles.D[epi % C::NUM_D_TILES], {row_block_idx*2 + cta_id, k_col_offset});
                    } else {
                        const int v_col_offset = col_offset - ((g.q_dim + g.k_dim) / (C::Nb / C::EPI_PIPE_DEPTH));
                        warpgroup::tma::store_async<dim::ROW, C::D_CACHE_POLICY>(
                            g.D_V, output_tiles.D[epi % C::NUM_D_TILES], {row_block_idx*2 + cta_id, v_col_offset});
                    }
                } else {
                    warpgroup::tma::store_async<dim::ROW, C::D_CACHE_POLICY>(
                        g.D, output_tiles.D[epi % C::NUM_D_TILES], {row_block_idx*2 + cta_id, C::EPI_PIPE_DEPTH * col_block_idx + epi});
                }
            }
        }
        warpgroup::sync(1);
