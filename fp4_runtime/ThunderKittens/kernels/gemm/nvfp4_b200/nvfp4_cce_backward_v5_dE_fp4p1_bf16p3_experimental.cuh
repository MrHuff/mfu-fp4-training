#pragma once
// ================================================================
// NVFP4 CCE Backward v5 dE — FP4 phase 1 + BF16 on-chip phase 3
//
...
// Phase 2: form logits-gradient G directly into BF16 SMEM staging
// Phase 3: dE[row_block, k_block] += G_bf16 @ C_col_bf16[k_block, vocab_block]^T
//
...
namespace nvfp4_cce_backward_v5_dE_fp4p1_bf16p3_experimental {
...
struct globals {
    ...
    using P3_B_fp4x2_tile = st_bf<C::Nb_out / C::CLUSTER_SIZE, C::Nb>;
    using P3_B_sc_tile    = st_bf<1, 1>;

    using G_fp4_row_tile = st_bf<C::Mb/2, C::Nb>;
    using G_sc_row_tile  = st_bf<1, 1>;
...
__device__ inline void kernel(const globals<C>& g) {
    ...
    __shared__ semaphore bf16_ready;
    __shared__ semaphore bf16_ready_cluster;
    if (threadIdx.x == 32) {
        ...
        init_semaphore(bf16_ready, 1, 0);
        init_semaphore(bf16_ready_cluster, 0, C::CLUSTER_SIZE);
    }
        ...
        if (warp_id == 3) {
            ...
            for (int block_idx = cluster_id; block_idx < num_blocks; block_idx += gridDim.x / C::CLUSTER_SIZE) {
                ...
                for (int vocab_block_idx = 0; vocab_block_idx < num_vocab_blocks; ++vocab_block_idx) {
                    ...
                    wait(bf16_ready, get_phasebit<1>(phasebits, 5));
            ...
            for (int block_idx = cluster_id; block_idx < num_blocks; block_idx += gridDim.x / C::CLUSTER_SIZE) {
                for (int vocab_block_idx = 0; vocab_block_idx < num_vocab_blocks; ++vocab_block_idx) {
                    ...
                    wait(bf16_ready_cluster, get_phasebit<0>(phasebits, 5));
                    tensor_after_thread_sync();

                    tma::expect_bytes(p3_tiles_arrived, 2 * sizeof(typename G::p3_tiles_t));
                    wait(p3_tiles_arrived, get_phasebit<0>(phasebits, 6));
                    mm2_ABt(accum, fp4_staging[phase].G_row, p3_tiles.B, p3_inputs_finished);
            ...
            for (int vocab_block_idx = 0; vocab_block_idx < num_vocab_blocks; ++vocab_block_idx) {
                ...
                for (int epi = 0; epi < C::EPI_PIPE_DEPTH; ++epi) {
                    ...
                    if (quant_row < C::Mb / 2) {
                        ...
                        #pragma unroll
                        for (int col = 0; col < (C::Nb / C::EPI_PIPE_DEPTH); ++col) {
                            bf16 value_bf;
                            move<bf16>::lds(value_bf, G::Out_tile::idx(d_base, {quant_row, col}));
                            move<bf16>::sts(
                                G::G_fp4_row_tile::idx(fp4_base, {quant_row, epi * (C::Nb / C::EPI_PIPE_DEPTH) + col}),
                                value_bf);
                        }
                    }
                ...
                if (warpgroup::warpid() == 0 && lane_id == 0) {
                    arrive(bf16_ready, 1);
                    if (cta_id == 0) {
                        arrive(bf16_ready_cluster, 1);
                    } else {
                        uint32_t local_addr = static_cast<uint32_t>(__cvta_generic_to_shared(&bf16_ready_cluster));
