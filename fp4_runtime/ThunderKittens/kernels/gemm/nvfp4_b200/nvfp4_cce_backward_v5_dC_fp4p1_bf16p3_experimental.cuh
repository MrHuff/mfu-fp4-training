#pragma once
// ================================================================
// NVFP4 CCE Backward v5 dC — FP4 phase 1 + BF16 on-chip phase 3
//
// Grid ownership: (vocab_superblock, k_superblock)
// One vocab superblock = 2 adjacent 128-row vocab blocks.
//
// For each row-block we:
//  1. recompute logits in FP4
//  2. form BF16 G^T directly into on-chip shared staging
//  3. reuse the staged Gt_super across 2 adjacent k blocks:
//       dC_super[k0] += Gt_super_bf16 @ E_col_bf16[k0]^T
//       dC_super[k1] += Gt_super_bf16 @ E_col_bf16[k1]^T
//
// This keeps the public 2-WG ownership/order contract but drops the
// on-the-fly NVFP4 requantization in the phase-2 -> phase-3 handoff.
// ================================================================

#include "nvfp4_cce.cuh"

namespace nvfp4_cce_backward_v5_dC_fp4p1_bf16p3_experimental {

__device__ __forceinline__ void store_b16_cluster_v5(uint32_t local_addr, int target_cta, uint16_t value) {
    ...
    asm volatile("{st.shared::cluster.b16 [%0], %1;}" :: "r"(addr), "h"(value));
}

__device__ __forceinline__ void store_b16_local_v5(uint32_t local_addr, uint16_t value) {
    asm volatile("{st.shared.b16 [%0], %1;}" :: "r"(local_addr), "h"(value));
}
...
struct debug_globals {
    ...
    using P3_B_fp4x2_tile = st_bf<C::Nb_out / C::CLUSTER_SIZE, C::Mb>;
    using P3_B_sc_tile    = st_bf<1, 1>;

    // BF16 stages the full reduction dimension explicitly instead of the
    // packed fp4x2 contract used by the public path.
    using Gt_fp4_row_tile = st_bf<C::Nb, C::Mb>;
    using Gt_sc_row_tile  = st_bf<1, 1>;
    ...
    using P3_B_fp4x2_gl     = gl<bf16,       1,  1, -1, -1, P3_B_fp4x2_tile>;
    ...
    if (warpgroup_id >= C::CONSUMER_WARPGROUPS && warp::elect_leader()) {
            ...
            for (int block_idx = cluster_id; block_idx < num_blocks; block_idx += gridDim.x / C::CLUSTER_SIZE) {
                ...
                for (int row_block_idx = 0; row_block_idx < num_row_blocks; ++row_block_idx) {
                    ...
                    for (int ksub = 0; ksub < 2; ++ksub) {
                        ...
                        mm2_ABt(p3_tm, fp4_staging.Gt_row, p3_tiles[ksub].B, p3_inputs_finished[ksub]);
        ...
        for (int block_idx = cluster_id; block_idx < num_blocks; block_idx += gridDim.x / C::CLUSTER_SIZE) {
            ...
            auto clear_a_super = [&]() {
                ...
                uint8_t *bf16_bytes = reinterpret_cast<uint8_t*>(&fp4_staging.Gt_row.data[0]);
                        ...
                        for (int epi = 0; epi < C::EPI_PIPE_DEPTH; ++epi) {
                            ...
                            for (int row16_pass = 0; row16_pass < LOCAL_ROW16_BLOCKS / ROW16_BLOCKS_PER_PASS; ++row16_pass) {
                                ...
                                for (int r = 0; r < 16; ++r) {
                                    ...
                                    const int staged_row = cta_id * (C::Mb / 2) + local_row_base + r;
                                    const uint32_t gt_addr = G::Gt_fp4_row_tile::idx(gt_fp4_base, {local_gc, staged_row});
                                    const uint16_t value_bits = *reinterpret_cast<uint16_t*>(&value_bf);
                                    if (subpass == cta_id) store_b16_local_v5(gt_addr, value_bits);
                                    else                  store_b16_cluster_v5(gt_addr, subpass, value_bits);
                                }
                            }
            ...
            if constexpr (Debug) {
                if (g.debug_gt_fp4_ptr && cluster_id == 0 && block_idx == debug_target_block) {
                    constexpr int CONSUMER_THREADS = WARPGROUP_WARPS * WARP_THREADS;
                    const uint8_t *gt_bytes = reinterpret_cast<const uint8_t*>(&fp4_staging.Gt_row.data[0]);
                    for (int idx = threadIdx.x; idx < (int)sizeof(typename G::Gt_fp4_row_tile); idx += CONSUMER_THREADS) {
                        g.debug_gt_fp4_ptr[cta_id * (int)sizeof(typename G::Gt_fp4_row_tile) + idx] = gt_bytes[idx];
                    }
                }
            }
        }
