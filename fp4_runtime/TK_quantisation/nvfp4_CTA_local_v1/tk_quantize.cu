...
tk_localcta_fused_norm_quantize(
    ...
    if (use_localcta_fused_direct_experimental()) {
        TORCH_CHECK(M % 128 == 0 && K % 128 == 0, "input M and K must be multiples of 128");
        auto [row_fp4, row_sc_prepared, col_fp4, col_sc_prepared, row_sg, col_sg] =
            allocate_quant_outputs_prepared(M, K, return_transpose, device);
        constexpr int BS = 256;
        localcta_compute_inv_rms_kernel<BS><<<M, BS, 0, stream>>>(
            reinterpret_cast<const __nv_bfloat16*>(input.data_ptr()),
            inv_rms.data_ptr<float>(),
            ...
            static_cast<int>(K));
        if (return_transpose) {
            launch_localcta_direct_norm_prepared<true>(
                input, gamma, inv_rms, with_silu,
                row_fp4, row_sc_prepared, col_fp4, col_sc_prepared, row_sg, col_sg);
        } else {
            launch_localcta_direct_norm_prepared<false>(
                input, gamma, inv_rms, with_silu,
                row_fp4, row_sc_prepared, col_fp4, col_sc_prepared, row_sg, col_sg);
        }
        auto err = cudaGetLastError();
        TORCH_CHECK(err == cudaSuccess, "tk_localcta_fused_norm_quantize failed: ",
                    cudaGetErrorString(err));
        return std::make_tuple(
            row_fp4, row_sc_prepared, col_fp4, col_sc_prepared, row_sg, col_sg, inv_rms
        );
    }
...
tk_localcta_silu_quantize_split_for_gemm(
    ...
    const int64_t H = h1_raw.size(1);
    if (use_localcta_fused_direct_experimental()) {
        TORCH_CHECK(M % 128 == 0 && H % 128 == 0, "h1_raw M and H must be multiples of 128");
        auto [row_fp4, row_sc_prepared, col_fp4, col_sc_prepared, row_sg, col_sg] =
            allocate_quant_outputs_prepared(M, H, true, h1_raw.device());
        launch_localcta_persistent_silu_split_prepared<true>(
            h1_raw, h3, row_fp4, row_sc_prepared, col_fp4, col_sc_prepared, row_sg, col_sg);
        auto err = cudaGetLastError();
...
tk_localcta_silu_deriv_quantize_split_for_gemm(
    ...
    const int64_t H = dh.size(1);
    if (use_localcta_fused_direct_experimental()) {
        TORCH_CHECK(M % 128 == 0 && H % 128 == 0, "dh M and H must be multiples of 128");
        auto [row_fp4_1, row_sc_prepared_1, col_fp4_1, col_sc_prepared_1, row_sg_1, col_sg_1] =
        ...
        launch_localcta_cluster_silu_deriv_split_prepared<true>(
            dh, h3, h1_raw,
            row_fp4_1, row_sc_prepared_1, col_fp4_1, col_sc_prepared_1, row_sg_1, col_sg_1,
            row_fp4_2, row_sc_prepared_2, col_fp4_2, col_sc_prepared_2, row_sg_2, col_sg_2);
        auto err = cudaGetLastError();
