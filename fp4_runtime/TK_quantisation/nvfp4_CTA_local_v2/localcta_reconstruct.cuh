...
__device__ __forceinline__ float sg_from_grid(
    ...
    const int chunk_row = row / 128;
    const int chunk_col = col / 128;
    return sg[chunk_row * sg_cols + chunk_col];
}
...
__global__ void reconstruct_rowwise_kernel(
    ...
    const float global_scale = sg_from_grid(sg, row, col, sg_rows, sg_cols);
