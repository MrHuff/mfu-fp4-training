...
__device__ __forceinline__ void v3_rowwise_scaling(
    ...
    for (int it = 0; it < V3_ITERATIONS_NORMAL; ++it) {
        ...
        const nvfp4_scale_t S_dec = compute_decoding_scaling_factor(block_amax, S_enc);
        const float coeff = compute_nvfp4_scaling_coefficient<float>(S_dec, S_enc);

        if (SF_storing) {
            sSFrowwise[stage_sc_Y + it * V3_THREADS_Y_ROWWISE][stage_sc_X] = S_dec;
        }
...
__device__ __forceinline__ void v3_colwise_scaling(
    ...
    const float bmax[2] = {
        static_cast<float>(__habs(amax_2x.x)),
        static_cast<float>(__habs(amax_2x.y))
    };

    #pragma unroll
    for (int w = 0; w < 2; ++w) {
        const nvfp4_scale_t S_dec = compute_decoding_scaling_factor(bmax[w], S_enc);
        sSFcolwise[sc_tr_Y + w][sc_tr_X] = S_dec;
        const float coeff = compute_nvfp4_scaling_coefficient<float>(S_dec, S_enc);
...
float scan_tile_amax(const IType* __restrict__ sIn_ptr, int buff_in) {
    ...
    for (int it = 0; it < V3_ITERATIONS_NORMAL; ++it) {
        ...
        for (int w = 0; w < V3_WAVES; ++w) {
            ...
            #pragma unroll
            for (int e = 0; e < V3_PACK_SIZE / 2; ++e) {
                float a = static_cast<float>(__habs(pairs[e].x));
                float b = static_cast<float>(__habs(pairs[e].y));
