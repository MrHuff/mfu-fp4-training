...
fused_group_quantize_kernel_dim0(
    ...
    // ─── Store rowwise scales to per-split buffer (TK swizzle format) ───
    {
    ...
    // ─── Store colwise scales to per-split buffer (TK swizzle format) ───
    if constexpr (RETURN_TRANSPOSE) {
...
fused_group_quantize_kernel_dim1(
    ...
    // Store rowwise scales — per-group buffer, using local column indexing
    {
