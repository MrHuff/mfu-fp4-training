...
// Fast path for full 128x128 chunks: 8 scales per row, packed into two 32-bit
// stores after the in-place read barrier.
__device__ __forceinline__ void swizzle_scales_row_full_inplace(
    nvfp4_scale_t* sSFrowwise_ptr
) {
    for (int row = threadIdx.x; row < (int)V3Config::CHUNK_DIM_Y; row += V3_THREADS) {
        const int j   = row % 32;
        const int grp = row / 32;
        const auto* src = reinterpret_cast<const uint8_t*>(sSFrowwise_ptr) + row * V3_SCALES_PER_CHUNK_X;
        const uint32_t packed0 = *reinterpret_cast<const uint32_t*>(src);
        const uint32_t packed1 = *reinterpret_cast<const uint32_t*>(src + 4);

        __syncthreads();

        auto* dst = reinterpret_cast<uint8_t*>(sSFrowwise_ptr);
        const int base = j * 16 + grp * 4;
        *reinterpret_cast<uint32_t*>(dst + base) = packed0;
        *reinterpret_cast<uint32_t*>(dst + 512 + base) = packed1;
...
__device__ __forceinline__ void swizzle_scales_col_full_inplace(
    nvfp4_scale_t* sSFcolwise_ptr
) {
    for (int col = threadIdx.x; col < (int)V3Config::CHUNK_DIM_X; col += V3_THREADS) {
        const int j   = col % 32;
        const int grp = col / 32;
        const auto* src = reinterpret_cast<const uint8_t*>(sSFcolwise_ptr) + col * V3_SCALES_PER_CHUNK_Y;
        const uint32_t packed0 = *reinterpret_cast<const uint32_t*>(src);
        const uint32_t packed1 = *reinterpret_cast<const uint32_t*>(src + 4);

        __syncthreads();

        auto* dst = reinterpret_cast<uint8_t*>(sSFcolwise_ptr);
        const int base = j * 16 + grp * 4;
        *reinterpret_cast<uint32_t*>(dst + base) = packed0;
        *reinterpret_cast<uint32_t*>(dst + 512 + base) = packed1;
...
__device__ __forceinline__ void quantize_and_store_chunk_pipelined_v5(
    ...
    for (int t = 0; t < V3_NUM_TILES; ++t) {
        ...
        // Protect the two FP4 output buffers before tile t reuses t - 2.
        if (t >= V3_BUFFS_NUM_OUT) {
            if (leading) {
...
// ═══════════════════════════════════════════════════════════════════
// Phase-2-only quantize kernel: takes PRE-COMPUTED amax, skips Phase 1.
// Use when amax was already computed by a fused silu+amax kernel on
// the same stream. No grid barrier needed — stream ordering guarantees
// amax is ready.
// ═══════════════════════════════════════════════════════════════════

struct Phase2Args {
    unsigned int* work_counter;
    float*        global_amax;     // pre-computed, read-only
    int tiles_X, tiles_Y, total_tiles;
    float* sg_output;
};
...
template <bool RETURN_TRANSPOSE, bool ENCODE_CENTRIC = true>
__global__ void __launch_bounds__(V3_THREADS)
persistent_quantize_phase2_kernel(
    const __grid_constant__ CUtensorMap tensor_map_input,
    const __grid_constant__ CUtensorMap tensor_map_output,
    const __grid_constant__ CUtensorMap tensor_map_output_t,
    const __grid_constant__ CUtensorMap tmap_scale_row,
    ...
    // Read pre-computed amax (set by silu+amax kernel on same stream)
    const float amax_val = args.global_amax[0];
    const float S_enc = compute_global_encode_scaling_factor_FP4(amax_val);
    ...
    // Work-stealing quantize loop (Phase 2 only)
    while (true) {
        __shared__ unsigned int s_chunk_id;
        ...
        for (int pre = 0; pre < min(2, (int)V3_NUM_TILES); ++pre) {
            const int ty = pre / V3_TILES_X, tx = pre % V3_TILES_X;
            if (leading) {
                ptx::mbarrier_arrive_expect_tx(&in_mbar[pre],
                    V3_BUFF_DIM_Y * V3_BUFF_DIM_X * sizeof(IType));
                ptx::cp_async_bulk_tensor_2d_global_to_shared(
                    reinterpret_cast<uint64_t*>(&sIn[pre]),
                    reinterpret_cast<const uint64_t*>(&tensor_map_input),
