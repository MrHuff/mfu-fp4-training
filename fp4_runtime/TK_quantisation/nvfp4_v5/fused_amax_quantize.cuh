...
// Each thread consumes at most 64 random words while quantizing one 128x128
// chunk (32 rowwise and 32 colwise).  Giving every logical chunk/thread a
// fixed counter lane keeps persistent work stealing deterministic while the
// invocation offset still advances between calls and CUDA-graph replays.
static constexpr uint64_t V3_SR_WORDS_PER_THREAD = 64;

struct V3StochasticRoundingState {
    uint64_t seed;
    uint64_t counter;

    __device__ __forceinline__ void init(
        const uint64_t* __restrict__ rng_state,
        uint64_t logical_chunk_id
    ) {
        seed = rng_state != nullptr ? rng_state[0] : 0;
        const uint64_t invocation_offset = rng_state != nullptr ? rng_state[1] : 0;
        const uint64_t sequence =
            logical_chunk_id * static_cast<uint64_t>(V3_THREADS) + threadIdx.x;
        counter = invocation_offset + sequence * V3_SR_WORDS_PER_THREAD;
    }
    ...
    __device__ __forceinline__ uint32_t next() {
        ...
        return static_cast<uint32_t>((value ^ (value >> 31)) >> 32);
    }
};
...
template <bool ENCODE_CENTRIC = true, bool USE_STOCHASTIC_ROUNDING = false>
__device__ __forceinline__ void v3_rowwise_scaling(
    ...
    V3StochasticRoundingState* sr_state = nullptr
) {
    ...
    for (int it = 0; it < V3_ITERATIONS_NORMAL; ++it) {
        ...
        for (int w = 0; w < V3_WAVES; ++w) {
            ...
            if constexpr (USE_STOCHASTIC_ROUNDING) {
                const uint32_t rbits03 = sr_state->next();
                const uint32_t rbits47 = sr_state->next();
                out = ptx::mul_cvt_bf16_to_fp4_8x_stochastic_rounding<float>(
                    e03, e47, coeff, rbits03, rbits47);
            } else {
                out = ptx::mul_cvt_bf16_to_fp4_8x_round_to_nearest<float>(
                    e03, e47, coeff);
            }
            const int sw = ((w + bank_group) * V3_PACK_SIZE) % V3_ELTS_PER_THREAD;
    ...
    for (int w = 0; w < 2; ++w) {
        ...
        for (int e = 0; e < V3_SCALE_DIM / 8; ++e) {
            ...
            if constexpr (USE_STOCHASTIC_ROUNDING) {
                ...
                rOut[e] = ptx::mul_cvt_bf16_to_fp4_8x_round_to_nearest<float>(
                    e03, e47, coeff);
            }
        }
