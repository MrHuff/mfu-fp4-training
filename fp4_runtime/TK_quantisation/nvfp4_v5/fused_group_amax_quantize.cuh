...
struct FusedGroupArgs {
    ...
    // Optional split-input TMA maps for dim=1 grouped quantization. When set,
    // split_range selects the map and global X is converted to split-local X.
...
fused_group_quantize_kernel_dim1(
    ...
    if constexpr (USE_STOCHASTIC_ROUNDING || COL_USE_STOCHASTIC_ROUNDING) {
        const uint64_t logical_chunk_id =
            static_cast<uint64_t>(ctaid_Y) * gridDim.x + ctaid_X;
        sr_state.init(args.rng_state, logical_chunk_id);
    }
    ...
    for (int t = 0; t < V3_NUM_TILES; ++t) {
        ...
        if (leading) {
            ...
            ptx::cp_async_bulk_tensor_2d_global_to_shared(
                ...
                input_block_offset_X + tx * V3_TILE_DIM_X, gy, &in_mbar[t]);
        }
    }
    ...
    const int chunk_cols = (
        use_split_input ? args.split_range[group_id + 1] : (int)cols
    ) - block_offset_X;
