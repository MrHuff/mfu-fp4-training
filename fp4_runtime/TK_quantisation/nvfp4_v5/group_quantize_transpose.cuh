...
struct MultiAmaxCastTransposeFusionArgs {
  ...
  int num_tensors;
  // Optional split-input source maps. If zero, kernel uses the single input tensor map arg.
  int num_input_tensor_maps;
...
__device__ __forceinline__ void CopyInputTileToShared(
    void *dst, const CUtensorMap *tensor_map_input0, const CUtensorMap *tensor_map_input1,
    MultiAmaxCastTransposeFusionArgs *kernel_args_ptr, const size_t chunk_X,
    const size_t global_chunk_Y, const size_t num_bytes, uint64_t *barrier,
    const bool is_master_thread) {
  if (kernel_args_ptr->num_input_tensor_maps <= 1) {
    copy_2d_to_shared(dst, tensor_map_input0, chunk_X, global_chunk_Y, num_bytes, barrier,
                      is_master_thread);
    return;
  }

  int tensor_id = GetTensorId(kernel_args_ptr, static_cast<int>(global_chunk_Y));
  tensor_id = min(tensor_id, 1);
  const size_t local_chunk_Y =
      global_chunk_Y - static_cast<size_t>(kernel_args_ptr->split_sections_range[tensor_id]);
  const CUtensorMap *src = (tensor_id == 0) ? tensor_map_input0 : tensor_map_input1;
  copy_2d_to_shared(dst, src, chunk_X, local_chunk_Y, num_bytes, barrier, is_master_thread);
}
...
constexpr size_t BUFFS_NUM = 4;
constexpr size_t PREFETCH_STAGES = (BUFFS_NUM - 1 < STAGES - 1) ? BUFFS_NUM - 1 : STAGES - 1;  // min(3, 3) = 3
...
__global__ void __launch_bounds__(THREADS_NUM)
    group_quantize_transpose_nvfp4_kernel(const __grid_constant__ CUtensorMap tensor_map_input0,
...
#pragma nv_diag_suppress static_var_with_dynamic_init
  __shared__ alignas(8) uint64_t mbar[BUFFS_NUM];

  initialize_barriers<BUFFS_NUM, THREADS_NUM>(mbar, is_master_thread);

  int IN_buff_readable_parity[BUFFS_NUM] = {};

  // Prefetch prologue: issue PREFETCH_STAGES initial TMA loads
#pragma unroll
  for (size_t pf = 0; pf < PREFETCH_STAGES; ++pf) {
    const size_t pf_buff = pf;
    const size_t pf_stage_offset_Y = pf * BUFF_DIM_Y;
    const size_t pf_global_offset_Y = block_offset_Y + pf_stage_offset_Y;
    CopyInputTileToShared(&in_sh[pf_buff * BUFF_IN_SIZE], &tensor_map_input0, &tensor_map_input1,
                          &kernel_args, block_offset_X, pf_global_offset_Y, shmem_buff_size,
                          &mbar[pf_buff], is_master_thread);
  }
...
#pragma unroll
  for (size_t stage = 0; stage < STAGES; ++stage) {
    ...
    // Prefetch next stage into ring buffer (deeper pipeline)
    if (stage + PREFETCH_STAGES < STAGES) {
      // Wait for prior TMA store to finish reading from the target buffer
      ptx::cp_async_bulk_wait_group_read<PREFETCH_STAGES>();

      const size_t next_pf_stage = stage + PREFETCH_STAGES;
      const size_t next_pf_buff = next_pf_stage % BUFFS_NUM;
      const size_t next_pf_offset_Y = next_pf_stage * BUFF_DIM_Y;
      const size_t pf_global_Y = block_offset_Y + next_pf_offset_Y;

      CopyInputTileToShared(&in_sh[next_pf_buff * BUFF_IN_SIZE], &tensor_map_input0,
                            &tensor_map_input1, &kernel_args, block_offset_X, pf_global_Y,
                            shmem_buff_size,
                            &mbar[next_pf_buff], is_master_thread);
    }

    ptx::fence_proxy_async_shared_cta();

    // Wait for the data to have arrived (parity-based barrier)
    ptx::mbarrier_wait_parity(&mbar[buff], IN_buff_readable_parity[buff]);
    IN_buff_readable_parity[buff] ^= 1;
...
#pragma unroll
      for (size_t it = 0; it < ITERATIONS_TRANSPOSE; ++it) {
        ...
        if constexpr (NO_ACTIVATIONS_NOT_FP32_INPUT) {
          IType block_amax_f16 = __float2bfloat16(0.0f);
#pragma unroll
          ...
          block_amax = __bfloat162float(block_amax_f16);
        } else {
#pragma unroll
          for (int i = 0; i < SCALE_DIM; ++i) {
            const int shmem_offset_colwise = shmem_offset_base_colwise_in + i * BUFF_IN_DIM_X;
...
#pragma unroll
          ...
          block_amax =
              __bfloat162float(__hmax(__habs(thread_amax_2x.x), __habs(thread_amax_2x.y)));
...
#pragma unroll
  ...
  if constexpr (RETURN_TRANSPOSE) {
    if (colwise_scale_is_within_bounds_Y && split_colwise_scale_ptr != nullptr) {
      ...
      if (kernel_args.swizzle_scales) {
        // Use TE's exact swizzle formula for colwise scales.
        // Layout: 512-byte tiles organized as (tile_m × num_tiles_k) grid.
        // Within each tile: byte_in_tile = j*16 + grp*4 + k_byte
        //   where j = row_in_tile % 32, grp = row_in_tile / 32
        const int local_scale_row = static_cast<int>(scales_offset_Y_t);
        const int tile_m = local_scale_row / 128;
        const int row_in_tile = local_scale_row % 128;
        const int j = row_in_tile % 32;
        const int grp = row_in_tile / 32;
        ...
        for (size_t s = 0; s < count; ++s) {
          const int local_scale_col = static_cast<int>(split_relative_X_t + s);
          ...
          reinterpret_cast<uint8_t*>(split_colwise_scale_ptr)[tile_start + byte_in_tile] =
              reinterpret_cast<const uint8_t&>(out_colwise_scales_sh[scale_idx_sh + s]);
        }
      } else {
  ...
  destroy_barriers<BUFFS_NUM>(mbar, is_master_thread);
#else
...
void group_quantize_transpose(const Tensor &input, const Tensor *noop,
  ...
  TRANSFORMER_ENGINE_SWITCH_CONDITION(
      ...
      TRANSFORMER_ENGINE_SWITCH_CONDITION(return_transpose, RETURN_TRANSPOSE, {
        ...
        kernel<<<grid, block_size, dshmem_size, stream>>>(tensor_map_input, tensor_map_input,
                                                          tensor_map_output, scales_ptr, noop_ptr, rows, cols,
