...
struct PersistentGroupArgs {
    ...
    // Optional split-input TMA maps for dim=1 grouped quantization.
    const CUtensorMap* input_tma_maps;  // device pointer, 8 bytes
    const uint64_t* rng_state;  // [seed, advancing invocation offset], optional
};
...
template <bool RETURN_TRANSPOSE, bool ENCODE_CENTRIC = true, int PHASE = 0>
...
persistent_group_quantize_kernel_dim0(
    ...
    static_assert(PHASE >= 0 && PHASE <= 2, "invalid grouped quantization phase");
    ...
    // PHASE 1: Scan amax (work-stealing). PHASE=1 exposes this as a
    // barrier-free producer; PHASE=2 consumes the completed amax values.
    if constexpr (PHASE != 2) while (true) {
    ...
    if constexpr (PHASE == 0) if (leading) {
        #pragma unroll
        for (int t = 0; t < V3_NUM_TILES; ++t) {
            ptx::mbarrier_invalid(&in_mbar[t]);
            ptx::mbarrier_init(&in_mbar[t], 1);
...
template <bool RETURN_TRANSPOSE, bool USE_STOCHASTIC_ROUNDING = false,
          bool ENCODE_CENTRIC = true,
          bool COL_USE_STOCHASTIC_ROUNDING = USE_STOCHASTIC_ROUNDING,
          int PHASE = 0>
__global__ void __launch_bounds__(V3_THREADS)
persistent_group_quantize_kernel_dim1(
    ...
    if constexpr (PHASE != 2) while (true) {
        ...
        const bool use_split_input = args.input_tma_maps != nullptr;
        const CUtensorMap* input_tma_map = use_split_input
            ? &args.input_tma_maps[gid]
            : &tensor_map_input;
        const int input_block_offset_X = use_split_input
            ? block_offset_X - col_group_start
            : block_offset_X;
        ...
        for (int t = 0; t < V3_NUM_TILES; ++t) {
            ...
            if (leading) {
                ...
                ptx::cp_async_bulk_tensor_2d_global_to_shared(
                    reinterpret_cast<uint64_t*>(&sIn[t]),
                    reinterpret_cast<const uint64_t*>(input_tma_map),
                    input_block_offset_X + tx * V3_TILE_DIM_X,
                    block_offset_Y + ty * V3_TILE_DIM_Y,
                    &in_mbar[t]);
            }
        }
    ...
    while (true) {
        ...
        const int chunk_cols = (
            use_split_input ? args.split_range[gid + 1] : (int)cols
        ) - block_offset_X;
        int buff_out = 0, buff_out_tr = 0;
        V3StochasticRoundingState sr_state{};
        if constexpr (USE_STOCHASTIC_ROUNDING || COL_USE_STOCHASTIC_ROUNDING) {
            sr_state.init(args.rng_state, static_cast<uint64_t>(chunk_id));
        }

        #pragma unroll
        for (int t = 0; t < V3_NUM_TILES; ++t) {
            const int stage_Y = t / V3_TILES_X, stage_X = t % V3_TILES_X;
            v3_rowwise_scaling<ENCODE_CENTRIC, USE_STOCHASTIC_ROUNDING>(
                               sIn_ptr, sOut_ptr, sSFrowwise_ptr,
                               S_enc, stage_Y, stage_X, t, buff_out, &sr_state);
            if constexpr (RETURN_TRANSPOSE)
                v3_colwise_scaling<ENCODE_CENTRIC, COL_USE_STOCHASTIC_ROUNDING>(
                                   sIn_ptr, sOut_tr_ptr, sSFcolwise_ptr,
                                   S_enc, stage_Y, stage_X, t, buff_out_tr, &sr_state);
            ptx::fence_proxy_async_shared_cta();
            __syncthreads();
            if (leading) {
                ptx::cp_async_bulk_tensor_2d_shared_to_global(
