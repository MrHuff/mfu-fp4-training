...
float scan_and_transform_tile(
    ...
    for (int it = 0; it < ITERS; ++it) {
        ...
        for (int e = 0; e < V3_ELTS_PER_THREAD; ++e) {
            ...
            float x_val = __bfloat162float(sIn[tile_idx][local_row][col_in_tile]);
            float g_val = __bfloat162float(gamma_smem[col_in_tile]);

            // Apply transform: rmsnorm ± silu
            float transformed = x_val * row_inv_rms * g_val;
            if constexpr (WITH_SILU) {
                transformed = device_silu(transformed);
            }

            // Write transformed bf16 back to SMEM (for Phase 2 quantize)
            sIn[tile_idx][local_row][col_in_tile] = __float2bfloat16_rn(transformed);

            tile_max = fmaxf(tile_max, fabsf(transformed));
        }
    }
