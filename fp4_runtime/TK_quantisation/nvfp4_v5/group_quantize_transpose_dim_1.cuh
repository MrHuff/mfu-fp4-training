...
#pragma unroll
  for (size_t pf = 0; pf < PREFETCH_STAGES; ++pf) {
    ...
    const size_t pf_offset_Y = block_offset_Y + pf * BUFF_DIM_Y;
    copy_2d_to_shared(&in_sh[pf_buff * BUFF_IN_SIZE], &tensor_map_input,
                       block_offset_X, pf_offset_Y, shmem_buff_size,
  ...
  for (size_t stage = 0; stage < STAGES; ++stage) {
    ...
    if (stage + PREFETCH_STAGES < STAGES) {
      ...
      copy_2d_to_shared(&in_sh[next_pf_buff * BUFF_IN_SIZE], &tensor_map_input,
                         block_offset_X,
                         block_offset_Y + next_pf_stage * BUFF_DIM_Y,
                         shmem_buff_size, &mbar[next_pf_buff], is_master_thread);
    }
