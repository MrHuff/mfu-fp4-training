...
struct TunableConfig {
  ...
  static constexpr int PREFETCH_STAGES = 3;
...
compute_nvfp4_scaling_coefficient<bf16>(const nvfp4_scale_t S_dec_block, const float S_enc) {
  ...
  return __float2bfloat16(scale_rcp);
}

__device__ __forceinline__ nvfp4_scale_t fp8e4m3_stochastic_round(float value,
                                                                  uint32_t rbits) {
  ...
  return out.x1;
#else
  NVTE_DEVICE_ERROR("fp8e4m3_stochastic_round is only supported on SM 10.0+.");
  return static_cast<nvfp4_scale_t>(value);
#endif
}

template <bool USE_SCALE_STOCHASTIC_ROUNDING>
__device__ __forceinline__ nvfp4_scale_t round_nvfp4_scale(float value, RNG_t &rng,
                                                           uint4 &random_uint4, int &rnd_idx) {
  value = fminf(value, detail::TypeExtrema<float>::max);
  if constexpr (USE_SCALE_STOCHASTIC_ROUNDING) {
    return fp8e4m3_stochastic_round(value, core::get_rbits(rng, random_uint4, rnd_idx));
  } else {
    return static_cast<nvfp4_scale_t>(value);
  }
}
...
compute_decoding_scaling_factor_maybe_sr(const float block_amax, const float S_enc, RNG_t &rng,
                                         uint4 &random_uint4, int &rnd_idx) {
  constexpr float fp4_max = detail::TypeExtrema<fp4e2m1>::max;
  const float S_dec_b = block_amax / fp4_max * S_enc;
  return round_nvfp4_scale<USE_SCALE_STOCHASTIC_ROUNDING>(S_dec_b, rng, random_uint4, rnd_idx);
}

template <bool USE_SCALE_STOCHASTIC_ROUNDING>
__device__ __forceinline__ nvfp4_scale_t
compute_encoding_scaling_factor_nv_maybe_sr(const float block_amax, const float S_enc, RNG_t &rng,
                                            uint4 &random_uint4, int &rnd_idx) {
  constexpr float fp4_max = detail::TypeExtrema<fp4e2m1>::max;
  if (block_amax <= 1.0e-9f) {
    return static_cast<nvfp4_scale_t>(detail::TypeExtrema<fp8e4m3>::max);
  }
  const float M = fp4_max / (block_amax * S_enc);
  return round_nvfp4_scale<USE_SCALE_STOCHASTIC_ROUNDING>(M, rng, random_uint4, rnd_idx);
}

template <bool USE_STOCHASTIC_ROUNDING, bool USE_FAST_MATH, bool ENCODE_CENTRIC = true,
          bool APPLY_RHT = false,
          bool USE_SCALE_STOCHASTIC_ROUNDING = false>
...
#pragma unroll
      for (int i = 0; i < SCALE_DIM; ++i) {
        rIn[w][i] = __float2bfloat16(vals[i]);
      }
    }
    thread_amax_2x = {__float2bfloat16(0.0f), __float2bfloat16(0.0f)};
#pragma unroll
    for (int i = 0; i < SCALE_DIM; ++i) {
      const IType2 elt_pair = {rIn[0][i], rIn[1][i]};
      ptx::abs_max_2x(thread_amax_2x, thread_amax_2x, elt_pair);
    }
  }
  const float block_amax[2] = {__bfloat162float(__habs(thread_amax_2x.x)),
  ...
  for (int w = 0; w < 2; ++w) {
    ...
    if constexpr (ENCODE_CENTRIC) {
        const nvfp4_scale_t S_mult_fp8 =
            compute_encoding_scaling_factor_nv_maybe_sr<USE_SCALE_STOCHASTIC_ROUNDING>(
                block_amax[w], S_enc_colwise, rng, random_uint4, rnd_idx);
        scale_inv = static_cast<float>(S_mult_fp8) * S_enc_colwise;
        S_b_fp8 = static_cast<nvfp4_scale_t>(1.0f / static_cast<float>(S_mult_fp8));
    } else {
        S_b_fp8 = compute_decoding_scaling_factor_maybe_sr<USE_SCALE_STOCHASTIC_ROUNDING>(
            block_amax[w], S_enc_colwise, rng, random_uint4, rnd_idx);
        constexpr float float_max = 3.4028235e+38f;
        const float S_dec_colwise = 1.0f / S_enc_colwise;
        scale_inv = fminf(1.0f / (static_cast<float>(S_b_fp8) * S_dec_colwise), float_max);
    }

    // Store decoding scale to SMEM buffer (R2S)
    sSFcolwise[scale_tr_offset_Y + w][scale_tr_offset_X] = S_b_fp8;

    // Use scale_inv directly as the quantization coefficient
    scaling_coeff_type SFcoefficient;
    if constexpr (std::is_same_v<scaling_coeff_type, float>) {
        SFcoefficient = scale_inv;
    } else {
        SFcoefficient = static_cast<scaling_coeff_type>(__float2bfloat16(fminf(scale_inv, detail::TypeExtrema<bf16>::max)));
    }
...
#pragma unroll
        for (int e = 0; e < PACK_SIZE / 2; ++e) {
          vals[swizzled_group_idx + 2 * e] = __bfloat162float(rIn[w][e].x);
          vals[swizzled_group_idx + 2 * e + 1] = __bfloat162float(rIn[w][e].y);
        }
      }
      fwht16(vals);
#pragma unroll
...
#pragma unroll
        for (int e = 0; e < PACK_SIZE / 2; ++e) {
          rIn[w][e].x = __float2bfloat16(vals[swizzled_group_idx + 2 * e]);
          rIn[w][e].y = __float2bfloat16(vals[swizzled_group_idx + 2 * e + 1]);
          ptx::abs_max_2x(thread_amax_2x, thread_amax_2x, rIn[w][e]);
        }
      }
    }
    ...
    if constexpr (ENCODE_CENTRIC) {
        const nvfp4_scale_t S_mult_fp8 =
            compute_encoding_scaling_factor_nv_maybe_sr<USE_SCALE_STOCHASTIC_ROUNDING>(
                block_amax, S_enc_rowwise, rng, random_uint4, rnd_idx);
        scale_inv = static_cast<float>(S_mult_fp8) * S_enc_rowwise;
        S_b_fp8 = static_cast<nvfp4_scale_t>(1.0f / static_cast<float>(S_mult_fp8));
    } else {
        S_b_fp8 = compute_decoding_scaling_factor_maybe_sr<USE_SCALE_STOCHASTIC_ROUNDING>(
            block_amax, S_enc_rowwise, rng, random_uint4, rnd_idx);
        constexpr float float_max = 3.4028235e+38f;
        const float S_dec_rowwise = 1.0f / S_enc_rowwise;
        scale_inv = fminf(1.0f / (static_cast<float>(S_b_fp8) * S_dec_rowwise), float_max);
    }
    ...
    if (SF_storing_thread) {
      ...
      sSFrowwise[scales_offset_Y][scales_offset_X] = S_b_fp8;
    }
...
template <bool USE_STOCHASTIC_ROUNDING, bool USE_SCALE_STOCHASTIC_ROUNDING>
__device__ __forceinline__ void advance_rowwise_rng_stream(RNG_t &rng, uint4 &random_uint4,
                                                           int &rnd_idx) {
#pragma unroll
  for (int it = 0; it < ITERATIONS_NORMAL; ++it) {
    if constexpr (USE_SCALE_STOCHASTIC_ROUNDING) {
      (void)core::get_rbits(rng, random_uint4, rnd_idx);
    }
    if constexpr (USE_STOCHASTIC_ROUNDING) {
#pragma unroll
      for (int w = 0; w < WAVES; ++w) {
...
template <bool USE_STOCHASTIC_ROUNDING, bool USE_FAST_MATH, bool RETURN_TRANSPOSE,
          bool ENCODE_CENTRIC = true, bool RETURN_ROW = true, bool ROW_RHT = false,
          bool COL_RHT = false, bool USE_SCALE_STOCHASTIC_ROUNDING = false,
          bool APPLY_RMSNORM = false, bool APPLY_SQRELU = false,
          bool APPLY_SQRELU_DERIV = false>
__global__ void __launch_bounds__(THREADS_NUM) quantize_transpose_nvfp4_tuned_1D_kernel(
    ...
    const bool swizzle_scales = false, const float *const inv_rms_ptr = nullptr,
    const IType *const gamma_ptr = nullptr, const IType *const sqrelu_aux_ptr = nullptr) {
#if (defined __CUDA_ARCH__) && (__CUDA_ARCH__ >= 1000)
  ...
  uint4 random_uint4 =
      (USE_STOCHASTIC_ROUNDING || USE_SCALE_STOCHASTIC_ROUNDING) ? rng.generate4()
                                                                 : uint4{0, 0, 0, 0};
  ...
  constexpr int out_mem_rowwise_data = RETURN_ROW ? buff_size_aligned_out : 0;
  constexpr int out_mem_colwise_data = RETURN_TRANSPOSE ? buff_size_aligned_out_t : 0;
  constexpr int out_mem_rowwise_scales = RETURN_ROW
      ? DIVUP_TO_MULTIPLE(
            TunableConfig::CHUNK_DIM_Y * SCALES_PER_CHUNK_X * sizeof(nvfp4_scale_t),
            TMA_SHMEM_ALIGNMENT)
      : 0;
...
#pragma unroll
    for (int stage = 0; stage < STAGES; ++stage) {
      ...
      if constexpr (APPLY_RMSNORM) {
        constexpr int pairs_per_row = BUFF_DIM_X / 2;
        constexpr int total_pairs = BUFF_DIM_Y * pairs_per_row;
        for (int pair_idx = threadIdx.x; pair_idx < total_pairs; pair_idx += THREADS_NUM) {
	          const int local_y = pair_idx / pairs_per_row;
	          const int local_x = (pair_idx - local_y * pairs_per_row) * 2;
	          const int global_y = block_offset_Y + stage_offset_Y + local_y;
	          const int global_x = block_offset_X + stage_offset_X + local_x;
	          const float2 x_val = __bfloat1622float2(
	              *reinterpret_cast<const __nv_bfloat162*>(&sIn[buff_in][local_y][local_x]));
	          const float2 g_val = __bfloat1622float2(
	              *reinterpret_cast<const __nv_bfloat162*>(gamma_ptr + global_x));
	          const float inv = inv_rms_ptr[global_y];
	          const float2 out = {x_val.x * inv * g_val.x, x_val.y * inv * g_val.y};
	          *reinterpret_cast<__nv_bfloat162*>(&sIn[buff_in][local_y][local_x]) =
	              __float22bfloat162_rn(out);
        }
        __syncthreads();
      }

      if constexpr (APPLY_SQRELU || APPLY_SQRELU_DERIV) {
        constexpr int pairs_per_row = BUFF_DIM_X / 2;
        constexpr int total_pairs = BUFF_DIM_Y * pairs_per_row;
        for (int pair_idx = threadIdx.x; pair_idx < total_pairs; pair_idx += THREADS_NUM) {
          const int local_y = pair_idx / pairs_per_row;
          ...
          const int global_y = block_offset_Y + stage_offset_Y + local_y;
          const int global_x = block_offset_X + stage_offset_X + local_x;
          const float2 in_val = __bfloat1622float2(
              *reinterpret_cast<const __nv_bfloat162*>(&sIn[buff_in][local_y][local_x]));
          float2 out;
          if constexpr (APPLY_SQRELU) {
            const float rx = fmaxf(in_val.x, 0.0f);
            const float ry = fmaxf(in_val.y, 0.0f);
            out = {rx * rx, ry * ry};
          } else {
            const float2 h_val = __bfloat1622float2(
                *reinterpret_cast<const __nv_bfloat162*>(
                    sqrelu_aux_ptr + global_y * cols + global_x));
            const float rx = fmaxf(h_val.x, 0.0f);
            const float ry = fmaxf(h_val.y, 0.0f);
            out = {2.0f * in_val.x * rx, 2.0f * in_val.y * ry};
          }
          *reinterpret_cast<__nv_bfloat162*>(&sIn[buff_in][local_y][local_x]) =
              __float22bfloat162_rn(out);
        }
      ...
      // NVFP4 Quantization
      if constexpr (RETURN_ROW) {
        rowwise_scaling<USE_STOCHASTIC_ROUNDING, USE_FAST_MATH, ENCODE_CENTRIC, ROW_RHT,
        ...
        // Preserve the previous full-row+col stochastic stream for col-only internal launches.
        advance_rowwise_rng_stream<USE_STOCHASTIC_ROUNDING, USE_SCALE_STOCHASTIC_ROUNDING>(
            rng, random_uint4, rnd_idx);
      }

      if constexpr (RETURN_TRANSPOSE) {
        colwise_scaling<USE_STOCHASTIC_ROUNDING, USE_FAST_MATH, ENCODE_CENTRIC, COL_RHT,
                        USE_SCALE_STOCHASTIC_ROUNDING>(
    ...
    // Vectorized store of scaling factors (S2G)
    {
      ...
      if constexpr (RETURN_ROW) {
        if (swizzle_scales) {
          ...
          // Within a row, k values [4*t..4*t+3] map to 4 consecutive bytes:
          //   base = tile_start + j*16 + group*4, offset = k % 4
          // So we pack 4 scale bytes into a uint32 and do a single 4-byte write.
          ...
          for (size_t row = threadIdx.x; row < TunableConfig::CHUNK_DIM_Y; row += THREADS_NUM) {
            ...
            if (row_global < rows) {
              ...
              // Vectorized: write 4 consecutive k-values as one uint32
              const int k_groups = count / 4;
              for (int kg = 0; kg < k_groups; ++kg) {
                const int k_base = kg * 4;
                const int k_global_base = scales_block_offset_X_rowwise + k_base;
                const int tile_k = k_global_base / 4;
                const int tile_start = (tile_m * num_tiles_k + tile_k) * 512;
                const int byte_in_tile = j * 16 + group * 4;
                // Pack 4 scale bytes into uint32
                uint32_t packed;
                uint8_t *p = reinterpret_cast<uint8_t*>(&packed);
                p[0] = reinterpret_cast<const uint8_t&>(sSFrowwise[row][k_base]);
                p[1] = reinterpret_cast<const uint8_t&>(sSFrowwise[row][k_base + 1]);
                p[2] = reinterpret_cast<const uint8_t&>(sSFrowwise[row][k_base + 2]);
                p[3] = reinterpret_cast<const uint8_t&>(sSFrowwise[row][k_base + 3]);
                *reinterpret_cast<uint32_t*>(
                    reinterpret_cast<uint8_t*>(scales_ptr) + tile_start + byte_in_tile) = packed;
              }
              // Handle remainder (count not divisible by 4)
              for (int k = k_groups * 4; k < count; ++k) {
      ...
      if constexpr (RETURN_TRANSPOSE) {
         if (swizzle_scales) {
          ...
          // but applied to the transposed scale tensor. Vectorized 4-byte stores.
          ...
          for (size_t row_tr = threadIdx.x; row_tr < TunableConfig::CHUNK_DIM_X;
            ...
            if (row_tr_global < cols) {
              ...
              for (int kg = 0; kg < k_groups; ++kg) {
                const int k_base = kg * 4;
                const int k_global_base = scales_block_offset_X_tr + k_base;
                const int tile_k = k_global_base / 4;
                const int tile_start = (tile_m * num_tiles_k_t + tile_k) * 512;
                const int byte_in_tile = j * 16 + group * 4;
                uint32_t packed;
                uint8_t *p = reinterpret_cast<uint8_t*>(&packed);
                p[0] = reinterpret_cast<const uint8_t&>(sSFcolwise[row_tr][k_base]);
                p[1] = reinterpret_cast<const uint8_t&>(sSFcolwise[row_tr][k_base + 1]);
                p[2] = reinterpret_cast<const uint8_t&>(sSFcolwise[row_tr][k_base + 2]);
                p[3] = reinterpret_cast<const uint8_t&>(sSFcolwise[row_tr][k_base + 3]);
                *reinterpret_cast<uint32_t*>(
                    reinterpret_cast<uint8_t*>(scales_t_ptr) + tile_start + byte_in_tile) = packed;
              }
              // Handle remainder
              for (int k = k_groups * 4; k < count; ++k) {
