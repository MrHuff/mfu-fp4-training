...
#pragma unroll
  for (int w = 0; w < 2; ++w) {
    ...
    if constexpr (ENCODE_CENTRIC) {
        const nvfp4_scale_t S_mult_fp8 = compute_encoding_scaling_factor_nv(block_amax[w], S_enc_colwise);
        scale_inv = static_cast<float>(S_mult_fp8) * S_enc_colwise;
        S_b_fp8 = static_cast<nvfp4_scale_t>(1.0f / static_cast<float>(S_mult_fp8));
    } else {
        S_b_fp8 = compute_decoding_scaling_factor(block_amax[w], S_enc_colwise);
...
#pragma unroll
    ...
    nvfp4_scale_t S_b_fp8;
    if constexpr (ENCODE_CENTRIC) {
        const nvfp4_scale_t S_mult_fp8 = compute_encoding_scaling_factor_nv(block_amax, S_enc_rowwise);
...
template <bool USE_STOCHASTIC_ROUNDING, bool USE_FAST_MATH, bool RETURN_TRANSPOSE, bool ENCODE_CENTRIC = true>
...
#pragma unroll
    for (int stage = 0; stage < STAGES; ++stage) {
      ...
      // NVFP4 Quantization
      rowwise_scaling<USE_STOCHASTIC_ROUNDING, USE_FAST_MATH, ENCODE_CENTRIC>(
          sIn_ptr, sOut_ptr, sSFrowwise_ptr, S_enc_rowwise, stage_Y, stage_X, buff_in, buff_out,
          rng, random_uint4, rnd_idx);

      if constexpr (RETURN_TRANSPOSE) {
        colwise_scaling<USE_STOCHASTIC_ROUNDING, USE_FAST_MATH, ENCODE_CENTRIC>(
