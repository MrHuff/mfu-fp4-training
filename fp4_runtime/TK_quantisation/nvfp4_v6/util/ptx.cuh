...
__device__ __forceinline__ void abs_max_2x_int(bf16x2 &dst, const bf16x2 &p1, const bf16x2 &p2) {
...
enum AmaxBackend : int {
  AMAX_BACKEND_NAIVE = 0,
  AMAX_BACKEND_XORSIGN = 1,
  AMAX_BACKEND_IMNMX = 2,
};

__device__ __forceinline__ uint32_t max_u32(uint32_t a, uint32_t b) {
#if (defined __CUDA_ARCH__) && (__CUDA_ARCH__ >= 1000)
  uint32_t out;
  asm volatile("max.u32 %0, %1, %2;" : "=r"(out) : "r"(a), "r"(b));
  return out;
#else
  NVTE_DEVICE_ERROR("max_u32 is only supported on SM 10.0+.");
  return 0;
#endif  // (defined __CUDA_ARCH__) && (__CUDA_ARCH__ >= 1000)
}

__device__ __forceinline__ uint32_t bf16x2_abs_bits(bf16x2 const &pair) {
  return reinterpret_cast<uint32_t const &>(pair) & 0x7fff7fffu;
}

__device__ __forceinline__ uint32_t bf16_low_bits(uint32_t bits) { return bits & 0x7fffu; }

__device__ __forceinline__ uint32_t bf16_high_bits(uint32_t bits) {
  return (bits >> 16) & 0x7fffu;
}

__device__ __forceinline__ void abs_max_2x_imnmx(uint32_t &lo_bits, uint32_t &hi_bits,
                                                 bf16x2 const &pair) {
  uint32_t const bits = bf16x2_abs_bits(pair);
  lo_bits = max_u32(lo_bits, bf16_low_bits(bits));
  hi_bits = max_u32(hi_bits, bf16_high_bits(bits));
}

__device__ __forceinline__ float bf16_bits_to_float(uint32_t bits) {
  bf16 value;
  reinterpret_cast<uint16_t &>(value) = static_cast<uint16_t>(bits);
  return __bfloat162float(value);
}

__device__ __forceinline__ float get_amax_of_bits(uint32_t lo_bits, uint32_t hi_bits) {
  return bf16_bits_to_float(max_u32(lo_bits, hi_bits));
}

__device__ __forceinline__ void abs_max_2x(bf16x2 &dst, const bf16x2 &p1, const bf16x2 &p2) {
  abs_max_2x_int(dst, p1, p2);
}
