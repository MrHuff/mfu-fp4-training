#!/usr/bin/env python3
"""Screen the native MXFP4 W2-residual -> exact RMS/quant boundary."""
...
DEFAULT_FP4_ROOT = Path(
    os.environ.get(
        "FP4_MXFP4_ROOT",
        os.environ.get("FP4_MATMUL_ROOT", "/opt/mfu/EXTERNAL_PATH"),
    )
)
DEFAULT_TK_ROOT = Path(
    os.environ.get("FP4_MATMUL_GEMM_ROOT", DEFAULT_FP4_ROOT / "ThunderKittens")
)
...
def main() -> None:
    ...
    parser.add_argument("--tk-root", type=Path, default=DEFAULT_TK_ROOT)
    ...
    gemm = _load_extension(
        "_C_mx",
        args.tk_root / "kernels/gemm/mxfp4_gb200" / f"_C_mx{suffix}",
    )
    tkq = _load_extension(
        "mxfp4_quant_v4",
        args.fp4_root / "TK_quantisation/mxfp4_v4" / f"mxfp4_quant_v4{suffix}",
    )
    ...
    x_fp4, x_sc = tkq.mxfp4_quantize_for_gemm(x, 1)
    w_fp4, w_sc = tkq.mxfp4_quantize_for_gemm(weight, 1)
    ...
    def baseline():
        gemm.mxfp4_gemm_residual_config(
            x_fp4, x_sc, w_fp4, w_sc, residual, out_base, 10
        )
        return tkq.mxfp4_fused_rmsnorm_quantize_row_and_col(
            out_base, gamma, args.epsilon, 1
        )

    def candidate():
        gemm.mxfp4_gemm_residual_rms(
            x_fp4, x_sc, w_fp4, w_sc, residual, out_cand, partial
        )
        return tkq.mxfp4_fused_rmsnorm_quantize_row_and_col_from_row_rms_partial(
            out_cand, gamma, partial, args.epsilon, 1
        )
    ...
    report: dict[str, object] = {
        ...
        "inv_rms_maxabs": float((base_result[4] - cand_result[4]).abs().max().item()),
        "inv_rms_byte_mismatch": _bytes_mismatch(base_result[4], cand_result[4]),
    ...
    if args.small_reference:
        partial_ref = out_cand.float().square().reshape(M, N // 256, 256).sum(-1)
        inv_ref = torch.rsqrt(out_cand.float().square().mean(-1) + args.epsilon)
        report["partial_maxabs_vs_torch"] = float((partial - partial_ref).abs().max().item())
        report["inv_maxabs_vs_torch"] = float((cand_result[4] - inv_ref).abs().max().item())

    if args.downstream_n:
        downstream_weight = torch.randn(
            (args.downstream_n, N), dtype=torch.bfloat16, device=device
        )
        next_fp4, next_sc = tkq.mxfp4_quantize_for_gemm(downstream_weight, 1)
        ...
        gemm.mxfp4_gemm(
            base_result[0], base_result[1], next_fp4, next_sc, downstream_base
        )
        gemm.mxfp4_gemm(
            cand_result[0], cand_result[1], next_fp4, next_sc, downstream_cand
        )
        torch.cuda.synchronize()
        report["downstream_n"] = args.downstream_n
        report["downstream"] = _bf16_metrics(downstream_base, downstream_cand)
    ...
    report.update({
        ...
        "stage_ms": {
            "baseline_residual_gemm": _event_ms(baseline_gemm, 2, stage_iterations),
            "candidate_residual_partial_gemm": _event_ms(candidate_gemm, 2, stage_iterations),
            "baseline_norm_quant": _event_ms(baseline_quant, 2, stage_iterations),
            "candidate_reduce_norm_quant": _event_ms(candidate_quant, 2, stage_iterations),
        },
    })
