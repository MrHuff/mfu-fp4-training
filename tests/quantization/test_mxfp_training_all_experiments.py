...
class PredictionTrainer:
    def __init__(self, model, train_loader, val_loader,optimizer_statement,loss,lr=1e-2,dtype = torch.bfloat16, epochs = 10, loss_scaling = False):
        self.model = model
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for X_batch, y_batch in tqdm(self.train_loader, desc="Training"):
                X_batch, y_batch = X_batch.to(self.dtype).to(self.device), y_batch.to(self.device)
            ...
            print(f"Epoch {epoch+1}/{self.epochs}: Train Acc = {self.train_losses[-1]:.4f}, Val Acc = {self.val_losses[-1]:.4f}")
...
def get_dataset_and_model(name="MNIST", checkpoint_folder='checkpoint'):
    ...
    if name == "MNIST":
        ...
        if 'te_fp4' in name:
            model = LlamaTrainerTEFP4(model_type=name,
                                    optimizer_eval_string=metaConfig['optimiser'],
                                    loss_scaling=metaConfig['loss_scaling'],
                                    checkpoint_folder = checkpoint_folder
                                    )
        else:
            model = LlamaTrainerWrapperBigger(model_type=name,
                                    optimizer_eval_string=metaConfig['optimiser'],
                                    loss_scaling=metaConfig['loss_scaling'],
                                    checkpoint_folder = checkpoint_folder
                                    )
        ...
        base_channels = 64 # Good starting point for 128x128, adjust based on VRAM/performance
    ...
    g = torch.Generator()
    g.manual_seed(42)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True,drop_last=True,num_workers=num_workers,worker_init_fn=worker_init_fn,pin_memory=True,generator=g,persistent_workers=True)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False,drop_last=True,num_workers=num_workers,worker_init_fn=worker_init_fn,pin_memory=True,generator=g,persistent_workers=True)
    return train_loader, val_loader, model, train_config

def orchestrateTraining(linearConfig, metaConfig, train_loader, val_loader, model,train_configs,checkpoint_folder='checkpoint_folder'):
    ...
    if metaConfig['dataset'] in ['gaussian_reg','MNIST','CIFAR10','IMAGENET100']:
        ...
        loss = torch.nn.MSELoss() if  metaConfig['dataset']=='gaussian_reg' else torch.nn.CrossEntropyLoss()
        trainer = PredictionTrainer(model,train_loader,val_loader, metaConfig['optimiser'],lr=train_configs['lr'],
                                    loss = loss,dtype=metaConfig['dtype'],epochs=train_configs['epochs'],
                                    loss_scaling=metaConfig['loss_scaling']
                                    )
        trainer.train()
        ...
        trainer = BigDiffusionTrainer(
            ...
            lr=train_configs['lr'],
            epochs=train_configs['epochs'], # Or more, training diffusion models takes time
            image_resolution=128,
            loss_scaling=metaConfig['loss_scaling']
        )
        trainer.train()
...
if __name__ == "__main__":
    ...
    train_loader, val_loader, model, train_configs = get_dataset_and_model(metaConfig['dataset'], checkpoint_folder)
    training_metrics,val_metrics = orchestrateTraining(linearConfig, metaConfig, train_loader, val_loader, model, train_configs, checkpoint_folder)

    # Only the main process saves the final result.
    if is_main_process:
        jobOutput = {'training_metrics': training_metrics,'val_metrics': val_metrics, **metaConfig}
