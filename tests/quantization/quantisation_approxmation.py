...
from low_bits_training.quantization.quantization_autograd_functions import SmoothQuantizeSigmoidOptimizedFunctionFP4, BaselineSmoothQuantFP4, SplineGradModule,QvalueQuantizer
...
if __name__ == "__main__":
    ...
    spline = SplineGradModule(quant_func=q_class.quantize, x_test=np.linspace(-6, 6, 40), dtype=x_tensor.dtype, K=1)
    ...
    ax.step(x, y_h, label="Hard Quantization (E2M1)", where='post', color='black', linewidth=2.5, zorder=5)

    # Plot the smooth approximations with improved styling
    ax.plot(x, y_g.detach(), label="Baseline Approx.", color='#d62728', linestyle='--', linewidth=2, zorder=4)
    ax.plot(x, y_s_sigmoid.detach(), label=fr"Sigmoid Approx. (T={temp})", color='#2ca02c', linestyle='-.', linewidth=2, zorder=3)
    ax.plot(x, y_s_spline.detach(), label="Spline Approx.", color='#9467bd', linestyle=':', linewidth=2.5, zorder=2)
    ...
    plt.tight_layout()
    plt.savefig(f'approx_quantisation_k={k}_torch_improved.pdf', bbox_inches='tight', dpi=300)
    plt.clf()
    ...
    ax.plot(x, grad_g.detach(), label=f"Baseline Gradient (k={k})", color='#d62728', linestyle='-', linewidth=1)
    ax.plot(x, grad_sigmoid.detach(), label=f"Sigmoid Gradient (T={temp})", color='#2ca02c', linestyle='-', linewidth=1)
    ax.plot(x, grad_spline.detach(), label="Spline Gradient", color='#9467bd', linestyle='-', linewidth=1)
    ...
    plt.tight_layout()
    plt.savefig(f'approx_quantisation_k={k}_torch_grad_improved.pdf', bbox_inches='tight', dpi=300)
    plt.clf()
    ...
    # Plot gradients with matching styles from Plot 1
    ax.plot(x, grad_g.detach(), label=f"Baseline Gradient (k={k})", color='#d62728', linestyle='-', linewidth=1)
    ax.plot(x, grad_sigmoid.clip(1.0,25).detach(), label=f"Sigmoid Gradient (T={temp})", color='#2ca02c', linestyle='-', linewidth=1)
    ax.plot(x, grad_spline.clip(1.0,25).detach(), label="Spline Gradient", color='#9467bd', linestyle='-', linewidth=1)

    # Improvement: Add a line at y=0 for reference
    ax.axhline(1, color='black', linestyle='-', linewidth=2, label='STE (Hard Quant.)')

    ax.set_title('Clipped Gradients of Differentiable Quantization Functions', fontsize=16)
    ax.set_xlabel('Input Value ($x$)', fontsize=12)
    ax.set_ylabel(r'Gradient ($\frac{dQ}{dx}$)', fontsize=12)
    ax.legend(loc='upper right', fontsize=10)
    ax.grid(True, linestyle='--', alpha=0.5)
    ...
    plt.tight_layout()
    plt.savefig(f'approx_quantisation_k={k}_torch_grad_improved_clipped.pdf', bbox_inches='tight', dpi=300)
    plt.clf()