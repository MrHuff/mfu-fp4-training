...
# --- NEW: Transformer Engine Imports ---
...
class BoundRecipeLinear(te.Linear):
    """
    A te.Linear layer that carries its own specific FP8 recipe.
    It ignores the global context and enforces its own recipe during the forward pass.
    """
    def __init__(self, in_features, out_features, bias=True, params_dtype=torch.bfloat16, recipe=None):
        super().__init__(in_features, out_features, bias=bias, params_dtype=params_dtype)
        self.bound_recipe = recipe

    def forward(self, inp):
        # If a recipe is bound, we force a nested context manager.
        # This overrides any outer global autocast context for this specific layer.
        if self.bound_recipe is not None:
            with te.fp8_autocast(enabled=True, fp8_recipe=self.bound_recipe):
                return super().forward(inp)
        else:
            # Fallback to standard behavior (checks global context)
            return super().forward(inp)
...
# --- NEW: Selective FP4 Replacement Logic ---

def rsetattr(obj, attr, val):
    pre, _, post = attr.rpartition('.')
    return setattr(getattr(obj, pre) if pre else obj, post, val)

def granular_replace_linear(
    model, 
    num_hidden_layers: int,
    mlp_recipe: te.recipe.Recipe,       # e.g. NVFP4
    attn_recipe: te.recipe.Recipe,      # e.g. MXFP4
    exclude_last_n_layers: int = 0,
    verbose: bool = True
):
    """
    Replaces Linear layers with BoundRecipeLinear, assigning specific recipes 
    to specific module types (MLP vs Attention).
    """
    
    modules_to_replace = []
    cutoff_index = num_hidden_layers - exclude_last_n_layers
    
    # Definition of what constitutes which category
    mlp_keywords = ["mlp.gate_proj", "mlp.up_proj", "mlp.down_proj"]
    attn_keywords = ["self_attn.q_proj", "self_attn.k_proj", "self_attn.v_proj", "self_attn.o_proj"]

    if verbose:
        print(f"🔍 Scanning...")
        print(f"   - MLP Strategy: {mlp_recipe.__class__.__name__}")
        print(f"   - Attn Strategy: {attn_recipe.__class__.__name__}")
    ...
    for name, module in model.named_modules():
        ...
        if "model.layers." in name:
            try:
                # Extract integer between 'layers.' and next '.'
                layer_idx = int(name.split('model.layers.')[1].split('.')[0])
            except (IndexError, ValueError):
                pass

        # 2. Identify Module Type and Assign Recipe
        ...
        # 3. Queue replacement if a recipe was assigned
        if selected_recipe is not None:
            modules_to_replace.append((name, module, selected_recipe))

    if verbose:
        print(f"⚡ Transformer Engine: Swapping {len(modules_to_replace)} modules to BoundRecipeLinear.")
    
    # 4. Execute Swap
    for name, module, recipe in modules_to_replace:
        # Instantiate our custom Bound wrapper
        te_layer = BoundRecipeLinear(
            in_features=module.in_features,
            out_features=module.out_features,
            bias=(module.bias is not None),
            params_dtype=torch.bfloat16,
            recipe=recipe  # <--- The Magic: Injection happens here
        )
...
class CustomOptimizerTrainer(Trainer):
    ...
    # --- MODIFIED: training_step with TE Autocast ---
...
class LlamaTrainerTEFP4:
    CONFIG_MAP = {
        'llama_te_fp4_9M': model9M, 'llama_te_fp4_60M': model60M, 'llama_te_fp4_350M': model350M,
        'llama_te_fp4_1B': model1B, 'llama_te_fp4_7B': model7B
    }
    ...
    def __init__(
        ...
        self.max_tokens = param_count * 100 if model_type in ['llama_te_fp4_9M','llama_te_fp4_60M'] else param_count*42
    ...
    def load_model(self):
        ...
        # --- TE MODIFICATION START: Layer Swapping ---
        # 1. Define cutoff (All layers - 4)
        n_layers = config_dict['num_hidden_layers']
        recipe_mlp = NVFP4BlockScaling() 
        recipe_attn = None
        # 2. Apply selective FP4 replacement (MLP only, not last 4)
        # This replaces nn.Linear with te.Linear
        self.model = granular_replace_linear(
            self.model, 
            num_hidden_layers=n_layers, 
            exclude_last_n_layers=4,
            mlp_recipe=recipe_mlp,   # <--- PASSED HERE
            attn_recipe=recipe_attn  # <--- PASSED HERE
        )        
        # 3. Move to device/dtype. TE layers initialize best when already in BF16.
        self.model.to(dtype=torch.bfloat16)
        # --- TE MODIFICATION END ---

        if 'SSWIG' in self.model_type:
            # Note: This must happen AFTER the swap so Swiglu wraps the te.Linear layers
    ...
    def run(self, config=None):
        ...
        is_main_process = int(os.environ.get("LOCAL_RANK", 0)) == 0
        if is_main_process:
            print("--- LlamaTrainerWrapperTE Run Started (DDP Mode) ---")

        self.train_dataset, self.val_dataset = self.load_and_tokenize_data()

        self.load_model()

        # --- TE NOTE: Replaced old swap_llama_mlp_to_mxlinear with replace_linear_with_te_fp4 in load_model ---
