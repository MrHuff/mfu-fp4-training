...
class SinusoidalPositionEmbeddings(nn.Module):
    ...
    def forward(self, time):
        ...
        embeddings = torch.exp(torch.arange(half_dim, device=device) * -embeddings).to(time.dtype)
...
class SmallGaussianDiffusion(nn.Module):
    ...
    def register_schedule(self, beta_start, beta_end):
        ...
        self.register_buffer("sqrt_alphas_cumprod", torch.sqrt(alphas_cumprod).to(self.dtype))
        self.register_buffer("sqrt_one_minus_alphas_cumprod", torch.sqrt(1. - alphas_cumprod).to(self.dtype))
    ...
    def q_sample(self, x0, t, noise=None):
        ...
        sqrt_one_minus = self.sqrt_one_minus_alphas_cumprod.gather(0, t).reshape(-1, 1, 1, 1)
    ...
    def sample(self, shape, device):
        ...
        for t_int in tqdm(reversed(range(self.timesteps)), desc="Sampling"):
            ...
            alpha_bar_prev = self.alphas_cumprod[t_int - 1] if t_int > 0 else torch.tensor(1.0, device=device)
...
class SmallDiffusionTrainer:
    # --- FIX 3 ---
    def __init__(self, model, diffusion, dataloader, optimizer_statement, sample_dir="samples", lr=1e-3, epochs=100,loss_scaling = False, val_loader=None):
    ...
    def train(self):
        for epoch in range(self.epochs):
            self.model.train()
            loss_val = 0.0
            for step, (images, _) in enumerate(tqdm(self.loader, desc=f"Epoch {epoch} [Train]")):
    ...
    def validate(self, epoch):
        ...
        val_loss_val = 0.0
        for images, _ in tqdm(self.val_loader, desc=f"Epoch {epoch} [Validate]", leave=False):
            images = images.to(self.diffusion.dtype).to(self.device)
            t = torch.randint(0, self.diffusion.timesteps, (images.size(0),), device=self.device)
            loss = self.diffusion.p_losses(images, t)
