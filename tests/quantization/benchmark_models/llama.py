    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        if self.optimizer_eval_string:
            try:
                ...
                raise ValueError(f"Error evaluating optimizer_eval_string: '{self.optimizer_eval_string}'. "
...
class LlamaTrainerWrapper:
    ...
    def __init__(
        ...
        if param_count is None:
            raise ValueError("Model config must include 'parameter_count' to compute training tokens.")
    ...
    def load_and_tokenize_data(self):
        ...
        cached_train_path = f"tokenized_train_seq{self.sequence_length}_max{self.max_tokens}.arrow"
        cached_test_path = f"tokenized_test_seq{self.sequence_length}_max{self.max_tokens}.arrow"
        ...
        if os.path.exists(cached_train_path) and os.path.exists(cached_test_path):
            ...
            print(f"    -> Loading from cache took {t_end_cache - t_start_cache:.4f} seconds.")
            
            epochs_needed = float(self.max_tokens * (0.9)) / float(len(train_ds) * self.sequence_length)
        ...
        print(f"    -> Loading raw dataset took {time.monotonic() - t_start:.4f} seconds.")
        ...
        print(f"    -> Tokenizing raw dataset took {time.monotonic() - t_start:.4f} seconds.")
        ...
        print(f"    -> Mapping to token IDs took {time.monotonic() - t_start:.4f} seconds.")
        
        t_start = time.monotonic()
        token_stream = truncate_token_lists(self.max_tokens, flat_token_ds["tokens"])
        print(f"Total flattened tokens: {len(token_stream)}")
        print(f"    -> Flattening and truncating token stream took {time.monotonic() - t_start:.4f} seconds.")
        ...
        print(f"    -> Chunking tokens into sequences took {time.monotonic() - t_start:.4f} seconds.")
        ...
        print(f"    -> Creating HF datasets from chunks took {time.monotonic() - t_start:.4f} seconds.")

        t_start = time.monotonic()
        train_chunked_ds.save_to_disk(cached_train_path)
        test_chunked_ds.save_to_disk(cached_test_path)
        print(f"    -> Saving datasets to disk took {time.monotonic() - t_start:.4f} seconds.")

        print(f"    -> Total data processing time: {time.monotonic() - t_start_processing:.4f} seconds.")
    ...
    def debug_train(self):
        ...
        print("Profiler will wait 5 steps, warmup for 1, and then record steps 7, 8, and 9.")
