...
class ImageTrainer:
    def __init__(self, model, train_loader, val_loader, lr=1e-3, epochs=20,dtype = torch.bfloat16, loss_scaler=False):
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            print(f"Epoch {epoch+1}/{self.epochs}: Train Acc = {self.train_accuracies[-1]:.4f}, Val Acc = {self.val_accuracies[-1]:.4f}")
...
SR = ['IntelFP4_exact']
HADAMARD= [None]
OPTIMISER_STATEMENT = ['StableSPAM(self.model.parameters(), lr=lr, adaclip =True)' ]
...
if __name__ == "__main__":
    ...
    batch_size = 512
    for scale_type, ma, step_gradient,sq,sr_mode,hada_mode,optimiser,ds,scale_round_mode,fp_scale_factor,loss_scaling,tensor_scaling,nh in itertools.product(SCALE_TYPES, MAX_APPROX, STEP_GRADIENTS,SCALING_QUANT_GRADIENTS,SR,HADAMARD,OPTIMISER_STATEMENT,DATASETS,SCALE_ROUNDING,FP_SCALING_FACTOR,LOSS_SCALING,USE_TENSOR_SCALING,NAN_HANDLING):
        ...
        model = (
            CNNModel(in_ch, out_ch)
            if ds == 'MNIST'
            else CNNModelCIFAR(in_ch, out_ch)
        )
        ...
        trainer = ImageTrainer(model, train_loader, val_loader,dtype=DTYPE,loss_scaler=loss_scaling)
        trainer.train()
