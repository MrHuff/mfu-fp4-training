...
class RegressionTrainer:
    def __init__(self, model,optimizer_statement,loss,lr=1e-2,dtype = torch.bfloat16, epochs = 20):
        self.model = model
        ...
        g = torch.Generator()
        g.manual_seed(42)
        train_loader = DataLoader(train_dataset, batch_size=4096, shuffle=True,drop_last=True,num_workers=4,worker_init_fn=worker_init_fn,pin_memory=True,generator=g)
        val_loader = DataLoader(val_dataset, batch_size=4096, shuffle=False,drop_last=True,num_workers=4,worker_init_fn=worker_init_fn,pin_memory=True,generator=g)
...
SCALE_TYPES = ['E4M3']
MAX_APPROX = ['hardsoftmax']
STEP_GRADIENTS = ['spline']
A_VALUES = [40]  # example alpha values
SCALING_QUANT_GRADIENTS = ['spline'] #['STE', 'baseline', 'spline', 'sigmoid'] #
SR = ['None_exact']
HADAMARD= ['forward_exact']
...
if __name__ == "__main__":
    ...
    for a in [0.5]:
        all_results = []
        for scale_type, ma, step_gradient,sq,sr_mode,hada_mode,optimiser,ds,scale_round_mode,fp_scale_factor,tensor_scaling in itertools.product(SCALE_TYPES, MAX_APPROX, STEP_GRADIENTS,SCALING_QUANT_GRADIENTS,SR,HADAMARD,OPTIMISER_STATEMENT,DATASETS,SCALE_ROUNDING,FP_SCALING_FACTOR,USE_TENSOR_SCALING):
            ...
            for a,k,tsg in itertools.product(a_list,k_list,TENSOR_SCALING_GRAD_list):
                ...
                trainer = RegressionTrainer(model,optimizer_statement=optimiser, loss = torch.nn.MSELoss())
                trainer.train()
