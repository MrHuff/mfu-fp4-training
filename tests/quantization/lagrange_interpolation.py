...
e2m1_values = np.array([-6.0, -4.0, -3.0, -2.0, -1.5, -1.0, -0.5, 0.0,
...
plt.plot(x_test, y_smooth, label="Smooth Quantization (Cubic Spline)", linestyle='--', color='b')
...
plt.figure(figsize=(8, 5))
plt.plot(x_test, y_derivative, label="Smooth Quantization (Cubic Spline) deriv", linestyle='--', color='b')
plt.xlabel("Input x")
...
y_torch = s(x_test_torch)

l = y_torch.sum().backward()
grad_s =  x_test_torch.grad


plt.figure(figsize=(8, 5))
plt.plot(x_test_torch.detach(), y_torch.detach(), label="Smooth Quantization (Cubic Spline)", linestyle='--', color='b')
plt.scatter(e2m1_values, e2m1_values, color='r', label="Hard Quantization Points")
plt.xlabel("Input x")
plt.ylabel("Quantized Output")
plt.legend()
...
plt.figure(figsize=(8, 5))
plt.plot(x_test_torch.detach(), grad_s, label="Smooth Quantization (Cubic Spline) deriv", linestyle='--', color='b')
plt.xlabel("Input x")
plt.ylabel("Splint derivative")
plt.legend()
plt.title("Cubic spline derivative")
